...
class Client extends EventEmitter {
  ...
  finalizeSession() { // Очищаем коллекцию сессий по токену и убиваем ссылку на сессии чтобы
                      // сборщик мусора их убрал.
...
class Server {
  ...
  listen(port) {
    this.httpServer.on("request", async (req, res) => { // Открываем порт
