...
Image → CNN → Feature Map → RPN → ~2000 region proposals
Time: T1

Stage 2: Classification
For each of ~2000 proposals:
  Crop region → ROI Pooling → FC layers → Classification + Box refinement
...
Image → CNN → Feature Map → Detection Layer → All predictions
...
Speedup: 5-25× faster than two-stage methods!
```
...
- Divides the image into a fixed grid (e.g., 7×7)
...
YOLO produces many predictions (grid_cells × anchors_per_cell). NMS removes duplicates to get clean final detections.
...
YOLO divides the input image into an S×S grid (commonly 7×7, 13×13, or 19×19 depending on version). Each grid cell is "responsible" for detecting objects whose center falls within that cell. The cell predicts bounding boxes and class probabilities for objects it's responsible for.

**Think of it like this:** Imagine dividing a city map into a 10×10 grid of neighborhoods. Each neighborhood's police station is responsible for crimes that happen in their area (determined by where the crime's center point is). When a crime is reported, the responsible station handles it. Similarly, each YOLO grid cell handles objects whose centers fall in that cell.
...
YOLO Grid Division (7×7):
┌───┬───┬───┬───┬───┬───┬───┐
│   │   │   │   │   │   │   │
├───┼───┼───┼───┼───┼───┼───┤
│   │   │ X │   │   │   │   │  X = Car 1 center
├───┼───┼───┼───┼───┼───┼───┤  → Cell (1,2) responsible
│   │   │   │   │   │ Y │   │
├───┼───┼───┼───┼───┼───┼───┤  Y = Car 2 center
│   │   │   │   │   │   │   │  → Cell (2,5) responsible
├───┼───┼───┼───┼───┼───┼───┤
│   │   │   │   │   │   │   │  Z = Person center
├───┼───┼───┼───┼───┼───┼───┤  → Cell (5,0) responsible
│ Z │   │   │   │   │   │   │
└───┴───┴───┴───┴───┴───┴───┘
...
Total per cell: 2 boxes × 5 values + 20 classes = 30 values
For 7×7 grid: 7×7×30 = 1,470 values total
```
...
```
YOLO Output Tensor: 7×7×30

Dimensions:
- 7×7: Grid cells (spatial)
...
┌─────────────────────────────────────────────────────────┐
│ Box 1 (5 values):                                       │
│ ├─ x: 0.0 to 1.0 (relative to cell)                    │
│ ├─ y: 0.0 to 1.0 (relative to cell)                    │
│ ├─ w: 0.0 to 1.0+ (relative to image width)            │
│ ├─ h: 0.0 to 1.0+ (relative to image height)           │
│ └─ confidence: 0.0 to 1.0                              │
├─────────────────────────────────────────────────────────┤
│ Box 2 (5 values):                                       │
│ ├─ x, y, w, h: Same format as Box 1                    │
│ └─ confidence                                          │
├─────────────────────────────────────────────────────────┤
│ Class Probabilities (20 values):                        │
│ └─ P(class_i | object) for i=1 to 20                   │
└─────────────────────────────────────────────────────────┘

Example values for cell (3,4):
[0.45, 0.67, 0.32, 0.28, 0.85,  ← Box 1: x,y,w,h,conf
 0.52, 0.61, 0.35, 0.30, 0.12,  ← Box 2: x,y,w,h,conf
 0.05, 0.02, 0.87, 0.01, ...]  ← Class probs (class 3 has 0.87)
```
...
The image is divided into a 7×7 grid. Each cell is a "zone of responsibility."
...
┌─────────────────────────────────────────┐
│  🚗      🧍   🚗        🚲            │
│  Car1   Person Car2    Bicycle          │
│                                         │
└─────────────────────────────────────────┘

YOLO divides into 7×7 grid:
...
For 448×448 image:
center_x_pixels = 0.514 × 448 = 230
center_y_pixels = 0.343 × 448 = 154
width_pixels = 0.3 × 448 = 134
height_pixels = 0.2 × 448 = 90
...
Final class confidence = Box_confidence × Class_probability

If box confidence = 0.9 and P(person|object) = 0.88:
Final person confidence = 0.9 × 0.88 = 0.792
...
```
YOLO Output: Tensor of shape (S, S, B×5 + C)

For YOLOv1: (7, 7, 2×5 + 20) = (7, 7, 30)

At grid cell (i,j):
Output[i,j, 0:5] = [bx₁, by₁, bw₁, bh₁, conf₁]
Output[i,j, 5:10] = [bx₂, by₂, bw₂, bh₂, conf₂]
Output[i,j, 10:30] = [P(c₁|obj), P(c₂|obj), ..., P(c₂₀|obj)]

Constraints:
- 0 ≤ bx, by ≤ 1 (within cell)
- 0 ≤ bw, bh (can be > 1 for large objects)
- 0 ≤ conf ≤ 1
- Σ P(cᵢ|obj) = 1 (probabilities sum to 1)
```
...
```
L_loc = λ_coord × Σ   Σ   𝟙ᵢⱼᵒᵇʲ [(xᵢ - x̂ᵢ)² + (yᵢ - ŷᵢ)²
                  i=0 j=0
...
**Symbol Legend:**
- `λ_coord`: Weight for coordinate loss (typically 5)
- `𝟙ᵢⱼᵒᵇʲ`: Indicator function (1 if cell i,j contains object, else 0)
- `(xᵢ, yᵢ)`: Predicted box center
- `(x̂ᵢ, ŷᵢ)`: Ground truth box center  
- `(wᵢ, hᵢ)`: Predicted box width/height
- `(ŵᵢ, ĥᵢ)`: Ground truth width/height
- `√`: Square root (makes loss more sensitive to small boxes)

**2. Confidence Loss (for cells WITH objects):**

```
L_conf_obj = Σ   Σ   𝟙ᵢⱼᵒᵇʲ (Cᵢ - Ĉᵢ)²
             i=0 j=0
```

Where:
- `Cᵢ`: Predicted confidence
- `Ĉᵢ`: Target confidence = IoU(predicted_box, ground_truth)

**3. No-Object Loss (for cells WITHOUT objects):**

```
L_conf_noobj = λ_noobj × Σ   Σ   𝟙ᵢⱼⁿᵒᵒᵇʲ (Cᵢ - 0)²
                         i=0 j=0
```

Where:
- `λ_noobj`: Weight for no-object loss (typically 0.5)
- `𝟙ᵢⱼⁿᵒᵒᵇʲ`: Indicator (1 if cell has NO object)
- Target is 0 (no object present)

**4. Classification Loss:**

```
L_class = Σ   Σ   𝟙ᵢⱼᵒᵇʲ Σ  (pᵢ(c) - p̂ᵢ(c))²
          i=0 j=0         c
```

Where:
- `pᵢ(c)`: Predicted probability of class c
- `p̂ᵢ(c)`: Ground truth (1 for correct class, 0 for others)

### Why Each Component Matters

**Coordinate Loss (with λ_coord = 5):**
...
Example:
Small box (w=0.1): Error of 0.05 → √0.1 - √0.15 = 0.32 - 0.39 = 0.07
Large box (w=0.8): Error of 0.05 → √0.8 - √0.85 = 0.89 - 0.92 = 0.03
...
Grid: 7×7
Ground truth: Car at center (224, 224), size 100×80 pixels
```
...
Grid position: (floor(0.5 × 7), floor(0.5 × 7)) = (3, 3)
...
Relative to cell:
bx = (0.5 - 3/7) / (1/7) = (0.5 - 0.429) / 0.143 = 0.497 ≈ 0.5
...
L_coord = λ_coord × [(0.48-0.5)² + (0.52-0.5)² 
                    + (√0.210-√0.223)² + (√0.165-√0.179)²]
        = 5 × [(0.02)² + (0.02)² + (0.458-0.472)² + (0.406-0.423)²]
        = 5 × [0.0004 + 0.0004 + 0.000196 + 0.000289]
        = 5 × 0.001285
        = 0.00643

Confidence loss:
IoU between predicted and ground truth box ≈ 0.82
L_conf = (0.88 - 0.82)² = 0.06² = 0.0036

Classification loss:
L_class = (0.02-0)² + (0.01-0)² + (0.91-1)² + ... (other 17 classes)
...
```
Image 448×448 with 7×7 grid overlay:
┌───┬───┬───┬───┬───┬───┬───┐
│   │   │   │   │   │   │   │
├───┼───┼───┼───┼───┼───┼───┤
│   │ 🚗│   │   │   │   │   │  Car 1 center in cell (1,1)
├───┼───┼───┼───┼───┼───┼───┤
│   │   │   │   │   │   │   │
├───┼───┼───┼───┼───┼───┼───┤
│   │   │   │   │   │🚗 │   │  Car 2 center in cell (3,5)
├───┼───┼───┼───┼───┼───┼───┤
│   │   │   │   │   │   │   │
├───┼───┼───┼───┼───┼───┼───┤
│   │   │🧍 │   │   │   │   │  Person center in cell (5,2)
├───┼───┼───┼───┼───┼───┼───┤
│   │   │   │   │   │   │   │
└───┴───┴───┴───┴───┴───┴───┘
...
- λ_coord = 5 weights coordinate errors heavily

Other 46 cells: No object
- Contribute to: no-object confidence loss only
- λ_noobj = 0.5 reduces their impact

Example calculation:
If cell (1,1) predicts car perfectly: contribution ≈ 0
If cell (1,1) predicts car poorly: contribution could be large
If cell (0,0) predicts no object: contribution ≈ 0
...
─────────────────────────────────────────────────────────────────────
Input   Image          448×448×3       -              -       -
1       Conv+ReLU      448×448×3       448×448×64     7×7     1
2       MaxPool        448×448×64      224×224×64     2×2     2
3       Conv+ReLU      224×224×64      224×224×192    3×3     1
4       MaxPool        224×224×192     112×112×192    2×2     2
5-8     Conv blocks    112×112×192     112×112×512    various 1
9       MaxPool        112×112×512     56×56×512      2×2     2
10-13   Conv blocks    56×56×512       56×56×1024     various 1
14      MaxPool        56×56×1024      28×28×1024     2×2     2
15-20   Conv blocks    28×28×1024      14×14×1024     various 1
21      MaxPool        14×14×1024      7×7×1024       2×2     2
22-23   Conv           7×7×1024        7×7×1024       3×3     1
24      Conv (final)   7×7×1024        7×7×30         1×1     1
─────────────────────────────────────────────────────────────────────

Final output: 7×7×30 (grid of predictions)
```
...
│ - Sigmoid for x,y,conf,classes    │
│   (squash to 0-1 range)           │
│ - Exponential or direct for w,h   │
│   (allow values > 1)              │
└────────────────────────────────────┘
```
...
x₁ = sigmoid(0.2) = 0.55
y₁ = sigmoid(1.5) = 0.82
conf₁ = sigmoid(2.1) = 0.89
class_probs = softmax([...])

After processing w, h:
w₁ = raw value or exp(raw) depending on implementation
h₁ = same
...
center_x = 0.514 × 448 = 230 pixels
center_y = 0.629 × 448 = 282 pixels

Step 3: Compute box size in pixels
box_width = 0.3 × 448 = 134 pixels
box_height = 0.2 × 448 = 90 pixels
...
```
For S×S grid with B boxes per cell:
Total predictions = S × S × B = 7 × 7 × 2 = 98 bounding boxes
...
∂L/∂Output[i,j, 0:5] = Coordinate gradients + Confidence gradient
∂L/∂Output[i,j, 5:10] = Confidence gradient only (for box 2)
∂L/∂Output[i,j, 10:30] = Classification gradients

For cell (i,j) WITHOUT object:
∂L/∂Output[i,j, 0:5] = Confidence gradient only (push to 0)
∂L/∂Output[i,j, 5:10] = Confidence gradient only (push to 0)
∂L/∂Output[i,j, 10:30] = Zero (no classification needed)
```
...
│   │   │   │   │   │   │   │
├───┼───┼───┼───┼───┼───┼───┤
│   │ ×│   │   │   │   │   │  ← Car 1, cell (1,1) responsible
├───┼───┼───┼───┼───┼───┼───┤
│   │   │   │   │   │   │   │
├───┼───┼───┼───┼───┼───┼───┤
│   │   │   │   │   │ ×│   │  ← Car 2, cell (3,5) responsible
├───┼───┼───┼───┼───┼───┼───┤
│   │   │   │   │   │   │   │
├───┼───┼───┼───┼───┼───┼───┤
│   │   │ ×│   │   │   │   │  ← Person, cell (5,2) responsible
├───┼───┼───┼───┼───┼───┼───┤
│   │   │   │   │   │   │   │
└───┴───┴───┴───┴───┴───┴───┘
...
```
∂L_coord/∂bx = 2 × λ_coord × (bx - bx_true)
             = 2 × 5 × (0.48 - 0.5)
             = 10 × (-0.02)
             = -0.2

∂L_coord/∂by = 2 × 5 × (0.52 - 0.5) = 0.2

For width (using square root):
∂L_coord/∂bw = 2 × 5 × (√bw - √bw_true) × (1/(2√bw))
             = 10 × (√0.21 - √0.223) × (1/(2√0.21))
             = 10 × (0.458 - 0.472) × (1/0.916)
             = 10 × (-0.014) × 1.092
             = -0.153
...
∂L_class/∂p₂ = 2 × (0.91 - 1.0) = -0.18

For class 0 (incorrect class):
∂L_class/∂p₀ = 2 × (0.02 - 0.0) = 0.04
...
∂L/∂coords[0,0] = 0 (don't care about box coordinates)
∂L/∂conf[0,0] = 2 × λ_noobj × (pred_conf - 0)
               = 2 × 0.5 × pred_conf
               = pred_conf
∂L/∂classes[0,0] = 0 (don't care about classes)

If predicted conf=0.02 (correctly low):
  ∂L/∂conf = 0.02 (small gradient, minor adjustment)

If predicted conf=0.8 (incorrectly high!):
  ∂L/∂conf = 0.8 (large gradient, big correction needed)
```
...
┌──────────────────┐         ┌──────────────────┐
│ ∂L/∂box_coords  │         │ ∂L/∂box = 0      │
│ ∂L/∂confidence  │         │ ∂L/∂conf (small) │
│ ∂L/∂classes     │         │ ∂L/∂classes = 0  │
│                  │         │                  │
│ Full gradients   │         │ Sparse gradients │
└──────────────────┘         └──────────────────┘
...
2. **Grid-Based:** Divide image into S×S grid
