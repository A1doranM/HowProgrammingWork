# One-Shot Learning and Siamese Networks

## Table of Contents

1. [One-Shot Learning](#one-shot-learning)
2. [Introduction to Siamese Networks](#introduction-to-siamese-networks)
3. [Siamese Network Architecture](#siamese-network-architecture)
4. [Similarity Learning](#similarity-learning)
5. [Training Siamese Networks](#training-siamese-networks)
6. [Detailed Examples](#detailed-examples)
7. [Forward Propagation](#forward-propagation)
8. [Backward Propagation](#backward-propagation)

---

## One-Shot Learning

### What is One-Shot Learning?

**Plain English Overview:**

One-shot learning is the ability to recognize or classify objects from just a single example. Unlike traditional machine learning that requires hundreds or thousands of examples per class, one-shot learning can learn from one (or very few) examples.

**Analogy:** Imagine meeting a new person at a party. After seeing their face just once, you can recognize them later in a crowd, even if they're wearing different clothes or the lighting is different. You don't need to see them 1,000 times to remember them - just once is enough. That's one-shot learning.

### The Face Recognition Problem

**Traditional Classification Problem:**

```
Training a normal face classifier:

Person 1: Need 100+ photos
Person 2: Need 100+ photos
Person 3: Need 100+ photos
Person 100: Need 100+ photos

Total: 10,000+ images for 100 people

Problems:
1. Need many images per person
2. Fixed number of people (100 classes)
3. Adding new person requires retraining entire network
4. Impractical for real-world face recognition systems
```

**Face Recognition Reality:**

```
Company with 10,000 employees:
- Each person has 1-2 photos in database
- New employees added daily
- Can't retrain network every time
- Need instant recognition with minimal examples

One-shot learning solves this!
```

**Visual Comparison:**

```
TRADITIONAL CLASSIFICATION:
Training:
Person A: [📷📷📷📷📷📷📷📷📷📷...100 photos]
Person B: [📷📷📷📷📷📷📷📷📷📷...100 photos]
Person C: [📷📷📷📷📷📷📷📷📷📷...100 photos]

Network learns: "These patterns = Person A, those = Person B"

Testing:
New photo → Network → "Person B" (from learned patterns)

Adding Person D:
Need 100 new photos + retrain entire network!


ONE-SHOT LEARNING:
Enrollment:
Person A: [📷] single photo → embedding vector [0.2, 0.8, 0.3, ...]
Person B: [📷] single photo → embedding vector [0.5, 0.1, 0.7, ...]
Person C: [📷] single photo → embedding vector [0.9, 0.3, 0.2, ...]

Store embeddings in database

Verification:
New photo → embedding → compare with stored embeddings → match/no match

Adding Person D:
Just compute embedding for their photo, add to database!
No retraining needed!
```

### Why Traditional Approaches Fail

**The Mathematical Problem:**

For traditional softmax classification with N people:

```
Final layer: FC with N outputs (one per person)
Output[i] = P(person is person_i)

Problems:
1. Fixed number of outputs N
2. Needs many examples per person to learn weights
3. New person = change architecture + retrain

Example:
100 people × 512 input features = 51,200 parameters in final layer
Each parameter needs sufficient data to train
→ Need many examples per person
```

**The One-Shot Solution:**

Instead of classifying to fixed categories, learn a similarity function:

```
Similarity(image1, image2) → score (0 to 1)

High score: Same person
Low score: Different people

No fixed categories!
Just compare pairs of images.
```

---

## Introduction to Siamese Networks

### What are Siamese Networks?

**Plain English Overview:**

A Siamese network is a neural network architecture with two identical sub-networks (twins) that share the exact same weights. These twins process two different inputs separately, then we compare their outputs to determine how similar the inputs are.

**Analogy:** Think of fingerprint matching. You have two fingerprint scanners (the twin networks) with identical technology. You scan the left thumb with one scanner and the right thumb with another scanner. Both scanners produce a numerical code (embedding) for the fingerprint pattern. Then you compare the codes - if they're similar, the fingerprints match. The scanners use the same technology (shared weights), but process different inputs.

**Visual Concept:**

```
SIAMESE NETWORK STRUCTURE:

Image 1 (Person A)           Image 2 (Person B)
     📷                           📷
     ↓                            ↓
┌─────────────┐             ┌─────────────┐
│  Network 1  │             │  Network 2  │
│             │             │             │
│  (Conv      │             │  (Conv      │
│   layers)   │  IDENTICAL  │   layers)   │  ← Same weights!
│             │    TWINS    │             │
│             │             │             │
│  Shared     │             │  Shared     │
│  Weights    │             │  Weights    │
└─────────────┘             └─────────────┘
     ↓                            ↓
Embedding 1:                Embedding 2:
[0.2, 0.7, 0.1, ...]       [0.5, 0.3, 0.8, ...]
     ↓                            ↓
     └──────────┬─────────────────┘
                │
          Compute Distance
                │
                ↓
            Distance = 0.72
                ↓
        If distance < threshold:
           "Same person"
        Else:
           "Different people"
```

### Why Siamese Networks Solve One-Shot Learning

**The Key Insight:**

Instead of learning to classify faces into categories, Siamese networks learn a similarity metric. They transform faces into embeddings where similar faces have nearby embeddings and different faces have distant embeddings.

**Traditional vs Siamese:**

```
TRADITIONAL CLASSIFIER:
Training: Learn patterns for each specific person
Testing: Match new photo to learned patterns
Problem: Can't handle people not seen during training!

Database has Person A, B, C
New person D appears → FAIL (not in training set)


SIAMESE NETWORK:
Training: Learn what makes faces similar/different
Testing: Compare embeddings of any two faces
Advantage: Works for NEW people not seen during training!

Training on: Person X, Y, Z (learning similarity metric)
Testing on: Person A, B, C, D (completely different people)
→ SUCCESS! Just needs to compare embeddings
```

**Why This Works:**

```
Network learns a general similarity function:

Training teaches:
"Same person images → similar embeddings (close in space)"
"Different person images → different embeddings (far in space)"

This knowledge transfers to new people!

Like learning to measure distances:
Once you know how to use a ruler, you can measure
ANY objects, not just the ones you practiced with.
```

---

## Siamese Network Architecture

### Twin Network Structure

**Complete Architecture:**

```
INPUT PAIR:
Image A (100×100×3)              Image B (100×100×3)
        ↓                                ↓
┌────────────────────────┐  ┌────────────────────────┐
│   ENCODER NETWORK      │  │   ENCODER NETWORK      │
│   (Shared Weights!)    │  │   (Shared Weights!)    │
│                        │  │                        │
│   Conv 3×3, 64        │  │   Conv 3×3, 64        │
│   ↓                    │  │   ↓                    │
│   Pool 2×2            │  │   Pool 2×2            │
│   ↓                    │  │   ↓                    │
│   Conv 3×3, 128       │  │   Conv 3×3, 128       │
│   ↓                    │  │   ↓                    │
│   Pool 2×2            │  │   Pool 2×2            │
│   ↓                    │  │   ↓                    │
│   Conv 3×3, 256       │  │   Conv 3×3, 256       │
│   ↓                    │  │   ↓                    │
│   Flatten             │  │   Flatten             │
│   ↓                    │  │   ↓                    │
│   FC → 128 dims       │  │   FC → 128 dims       │
└────────────────────────┘  └────────────────────────┘
        ↓                                ↓
    Embedding A                     Embedding B
    [e₁, e₂, ..., e₁₂₈]           [f₁, f₂, ..., f₁₂₈]
        ↓                                ↓
        └────────────┬───────────────────┘
                     ↓
            DISTANCE COMPUTATION
                     ↓
        d = ||Embedding_A - Embedding_B||
                     ↓
              Distance = 2.47
                     ↓
        If d < threshold (e.g., 0.5):
            Output: "SAME person"
        Else:
            Output: "DIFFERENT people"
```

**Weight Sharing:**

```
Critical property: Both networks share the EXACT SAME weights!

Network 1 weights = Network 2 weights

Why?
- Ensures consistent embeddings
- Same face always maps to same embedding
- Embedding(Face_A, Network1) = Embedding(Face_A, Network2)

Implementation:
Actually use ONE network, pass images through it sequentially
Or create two network instances with tied weights
```

**Mathematical Formulation:**

```
Let f(x; θ) be the encoder network with parameters θ

For image pair (x₁, x₂):

Embedding₁ = f(x₁; θ)  ∈ ℝᵈ
Embedding₂ = f(x₂; θ)  ∈ ℝᵈ

Where:
- θ: Shared network parameters (weights)
- d: Embedding dimension (e.g., 128)
- f(): Encoder function (series of conv, pool, FC layers)

Distance:
D(x₁, x₂) = ||f(x₁; θ) - f(x₂; θ)||

Common distance metrics:
- L2: ||e₁ - e₂||₂ = √(Σᵢ(e₁ᵢ - e₂ᵢ)²)
- L1: ||e₁ - e₂||₁ = Σᵢ|e₁ᵢ - e₂ᵢ|
- Cosine: 1 - (e₁·e₂)/(||e₁|| ||e₂||)
```

### Embedding Layer

**Purpose:**

The embedding layer transforms the input image into a fixed-dimensional vector (embedding) that captures the essential features of the face in a compact representation.

**Properties of Good Embeddings:**

```
1. Same person → Similar embeddings
   f(person_A_photo1) ≈ f(person_A_photo2)
   Small distance

2. Different people → Different embeddings
   f(person_A) ≠ f(person_B)
   Large distance

3. Compact: 128-512 dimensions (vs 100×100×3 = 30,000 input dimensions)

4. Meaningful: Nearby embeddings = similar faces
```

**Visual Embedding Space:**

```
2D projection of 128D embedding space:

      ↑
      │
    P₁│  × ×        Person 1's photos
      │  × ×        clustered together
      │
    P₂│        🔷 🔷  Person 2's photos
      │        🔷     clustered together
      │
    P₃│              ■ ■  Person 3's photos
      │              ■    clustered together
      │
      └────────────────────────────→

Key properties:
- Photos of same person cluster together
- Different people's clusters are separated
- Distance between clusters > distance within cluster
```

### Distance Computation

**L2 Distance (Euclidean):**

Most common in Siamese networks.

```
For embeddings e₁ = [e₁₁, e₁₂, ..., e₁ₙ] and e₂ = [e₂₁, e₂₂, ..., e₂ₙ]:

L2_distance = √(Σᵢ(e₁ᵢ - e₂ᵢ)²)

Step-by-step:
1. Subtract: diff = e₁ - e₂
2. Square each element: diff²
3. Sum: Σ diff²
4. Square root: √sum

Example:
e₁ = [0.5, 0.8, 0.3]
e₂ = [0.6, 0.7, 0.4]

diff = [0.5-0.6, 0.8-0.7, 0.3-0.4] = [-0.1, 0.1, -0.1]
diff² = [0.01, 0.01, 0.01]
sum = 0.03
distance = √0.03 = 0.173
```

---

## Similarity Learning

### Learning to Measure Similarity

**The Goal:**

Train the network so that:
```
Distance(same_person_photo1, same_person_photo2) < threshold
Distance(different_person_photo1, different_person_photo2) > threshold
```

**How Learning Works:**

```
Initially (random weights):
Person A photo 1 → [random embedding]
Person A photo 2 → [different random embedding]
Distance = large (BAD! Same person should be close)

Person A photo 1 → [random embedding]
Person B photo 1 → [different random embedding]
Distance = could be small (BAD! Different people should be far)

After training:
Person A photo 1 → [0.2, 0.7, 0.5, ...]
Person A photo 2 → [0.21, 0.69, 0.51, ...] ← Very similar!
Distance = 0.02 (GOOD! Same person, small distance)

Person A photo 1 → [0.2, 0.7, 0.5, ...]
Person B photo 1 → [0.8, 0.1, 0.9, ...] ← Very different!
Distance = 0.95 (GOOD! Different people, large distance)
```

### Embedding Space Concept

**What is Embedding Space?**

A high-dimensional space (e.g., 128 dimensions) where each face is represented as a point. The network learns to position points such that:
- Similar faces are close together
- Different faces are far apart

**Visual 2D Projection:**

```
Embedding Space (128D, shown in 2D):

        Dimension 2
            ↑
            │
      0.8   │     A2
            │  A1  ×
            │  ×
      0.6   │
            │          B1
      0.4   │      B2  ○
            │       ○
      0.2   │
            │               C1
      0.0   │              C2 ◆
            │               ◆
            └────────────────────────→ Dimension 1
            0.0   0.2  0.4  0.6  0.8

Where:
A1, A2 = Two photos of Person A (close together)
B1, B2 = Two photos of Person B (close together)
C1, C2 = Two photos of Person C (close together)

Distance within person: small (~0.1)
Distance between people: large (~0.5-0.8)
```

### Distance Metrics

**L1 Distance (Manhattan):**

```
d_L1 = Σᵢ |e₁ᵢ - e₂ᵢ|

Geometric interpretation: Distance traveling along grid lines

Example:
e₁ = [0.5, 0.8, 0.3]
e₂ = [0.6, 0.7, 0.4]

d_L1 = |0.5-0.6| + |0.8-0.7| + |0.3-0.4|
     = 0.1 + 0.1 + 0.1
     = 0.3
```

**L2 Distance (Euclidean):**

```
d_L2 = √(Σᵢ(e₁ᵢ - e₂ᵢ)²)

Geometric interpretation: Straight-line distance

Example (same vectors):
e₁ = [0.5, 0.8, 0.3]
e₂ = [0.6, 0.7, 0.4]

d_L2 = √((0.1)² + (0.1)² + (0.1)²)
     = √0.03
     = 0.173
```

**Cosine Similarity:**

```
similarity = (e₁ · e₂) / (||e₁|| ||e₂||)
distance = 1 - similarity

Measures angle between vectors, not magnitude

Example:
e₁ = [0.5, 0.8, 0.3]
e₂ = [0.6, 0.7, 0.4]

e₁ · e₂ = 0.5×0.6 + 0.8×0.7 + 0.3×0.4 = 0.3 + 0.56 + 0.12 = 0.98
||e₁|| = √(0.25 + 0.64 + 0.09) = √0.98 = 0.99
||e₂|| = √(0.36 + 0.49 + 0.16) = √1.01 = 1.00

similarity = 0.98 / (0.99 × 1.00) = 0.99
distance = 1 - 0.99 = 0.01 (very similar!)
```

### What Makes Good Embeddings

**Desired Properties:**

1. **Discriminative:** Different people → far apart embeddings
2. **Invariant:** Same person (different poses/lighting) → close embeddings
3. **Compact:** Low dimensionality (128-512 dims)
4. **Normalized:** Often unit vectors for cosine similarity

**Training Goal:**

```
Minimize intra-class distance (same person):
d(person_i_photo1, person_i_photo2) → small

Maximize inter-class distance (different people):
d(person_i, person_j) → large

Margin:
inter_class_distance - intra_class_distance > margin
Different people should be farther apart than threshold
```

---

## Training Siamese Networks

### Contrastive Loss Function

**Plain English:**

Contrastive loss trains the network by:
- Making embeddings close for similar pairs (same person)
- Making embeddings far for dissimilar pairs (different people)

**Mathematical Formula:**

```
L_contrastive = (1/2N) × Σ [y × d² + (1-y) × max(margin - d, 0)²]
                        pairs

Symbol Legend:
- N: Number of pairs
- y: Label (1 if same person, 0 if different)
- d: Distance between embeddings
- margin: Minimum desired distance for different people (e.g., 1.0)
- max(margin - d, 0): Penalty when different pairs are too close
```

**Breaking Down the Formula:**

```
For SIMILAR pair (same person, y=1):
Loss = (1/2) × d²

Minimizes distance:
- d=0 → loss=0 (perfect! embeddings identical)
- d=0.5 → loss=0.125 (okay, somewhat close)
- d=2.0 → loss=2.0 (bad! same person should be closer)

For DISSIMILAR pair (different people, y=0):
Loss = (1/2) × max(margin - d, 0)²

Penalizes if distance < margin:
- d=2.0, margin=1.0 → loss=0 (good! far enough)
- d=0.5, margin=1.0 → loss=(1.0-0.5)²/2 = 0.125 (penalty for being too close)
- d=0.1, margin=1.0 → loss=(1.0-0.1)²/2 = 0.405 (large penalty!)
```

**Example Calculation:**

```
Pair 1 (same person):
e₁ = [0.5, 0.8, 0.2]
e₂ = [0.52, 0.79, 0.21]
d = 0.0245
y = 1

Loss₁ = (1/2) × (0.0245)² = 0.0003

Pair 2 (different people):
e₁ = [0.5, 0.8, 0.2]
e₃ = [0.9, 0.1, 0.7]
d = 0.74
y = 0
margin = 1.0

Loss₂ = (1/2) × max(1.0 - 0.74, 0)²
      = (1/2) × (0.26)²
      = 0.0338

Total Loss = (Loss₁ + Loss₂) / 2 = 0.017
```

### Triplet Loss Function

**Plain English:**

Triplet loss uses three images at once:
- Anchor: Reference image
- Positive: Same person as anchor
- Negative: Different person from anchor

Goal: Make distance(anchor, positive) much smaller than distance(anchor, negative).

**Visual Concept:**

```
TRIPLET TRAINING EXAMPLE:

Anchor           Positive         Negative
(Person A,       (Person A,       (Person B)
 photo 1)         photo 2)
   📷               📷              📷
   ↓                ↓               ↓
   └────────┬───────┘               │
            ↓                        ↓
        Should be CLOSE          Should be FAR
            ↓                        ↓
     d(A,P) = small           d(A,N) = large
         
Goal: d(A,N) - d(A,P) > margin

If already satisfied: loss = 0
If not satisfied: push them apart more
```

**Mathematical Formula:**

```
L_triplet = Σ max(d(A,P) - d(A,N) + margin, 0)
           triplets

Symbol Legend:
- A: Anchor image
- P: Positive image (same person as A)
- N: Negative image (different person from A)
- d(x,y): Distance between embeddings of x and y
- margin: Desired separation (e.g., 0.2)
- max(..., 0): Only penalize if margin violated
```

**Breaking Down:**

```
Ideal case:
d(A,P) = 0.1  (anchor and positive close)
d(A,N) = 0.9  (anchor and negative far)
margin = 0.2

d(A,N) - d(A,P) = 0.9 - 0.1 = 0.8 > 0.2 ✓
Loss = max(0.1 - 0.9 + 0.2, 0) = max(-0.6, 0) = 0

Good case:
d(A,P) = 0.2
d(A,N) = 0.5
margin = 0.2

d(A,N) - d(A,P) = 0.5 - 0.2 = 0.3 > 0.2 ✓
Loss = max(0.2 - 0.5 + 0.2, 0) = max(-0.1, 0) = 0

Bad case (needs correction):
d(A,P) = 0.4
d(A,N) = 0.5
margin = 0.2

d(A,N) - d(A,P) = 0.5 - 0.4 = 0.1 < 0.2 ✗
Loss = max(0.4 - 0.5 + 0.2, 0) = max(0.1, 0) = 0.1

Network will adjust to push N farther or pull P closer
```

### Positive and Negative Pairs

**Creating Training Pairs:**

**For Contrastive Loss:**

```
Dataset: 100 people, 10 photos each = 1,000 images

Create pairs:

Positive pairs (same person):
Person 1: (photo1, photo2), (photo1, photo3), ..., (photo9, photo10)
         → (10 choose 2) = 45 pairs per person
         × 100 people = 4,500 positive pairs

Negative pairs (different people):
Person 1 photo 1 + Person 2 photo 1
Person 1 photo 1 + Person 3 photo 1
...
Approximately 100 × 100 × 10 × 10 = 1,000,000 possible pairs
Sample subset: 10,000 negative pairs

Total training pairs: 4,500 positive + 10,000 negative = 14,500 pairs
```

**For Triplet Loss:**

```
Create triplets (Anchor, Positive, Negative):

Person 1: Anchor=photo1, Positive=photo2, Negative=Person2_photo1
Person 1: Anchor=photo1, Positive=photo3, Negative=Person3_photo1
...

Per person: 9 positives × 99 negatives = 891 triplets
× 100 people = 89,100 triplets

Sample subset for efficient training
```

### Hard Negative Mining

**The Problem:**

Most negative pairs are too easy (very different faces):

```
Easy negative: 
Anchor: Young woman
Negative: Old man with beard
Distance: 2.5 (very far)
Loss: 0 (already satisfies margin)

Network learns nothing from easy negatives!
```

**Hard Negative Mining:**

Select negatives that are difficult:

```
Hard negative:
Anchor: Person A
Negative: Person B who looks similar to A
Distance: 0.35 (too close for comfort!)
Loss: >0 (violates margin)

Network forced to learn subtle differences!
```

**Strategy:**

```
1. Online hard negative mining:
   During training, from a batch:
   - Compute all embeddings
   - For each anchor, find hardest negative
     (different person but closest embedding)
   - Use these hard negatives for loss

2. Semi-hard negative mining:
   Select negatives where:
   d(A,P) < d(A,N) < d(A,P) + margin
   
   Not too easy (d(A,N) >> d(A,P) + margin)
   Not too hard (d(A,N) < d(A,P))
```

---

## Detailed Examples

### Example 1: Simple Pair Comparison

**Scenario:** Verify if two face images are the same person

**Given:**
```
Image 1: 64×64×3 (Person A, frontal)
Image 2: 64×64×3 (Person A, side profile)
```

**Encoder Network:**
```
Input: 64×64×3
Conv 3×3, 32 filters → 64×64×32
MaxPool 2×2 → 32×32×32
Conv 3×3, 64 filters → 32×32×64
MaxPool 2×2 → 16×16×64
Conv 3×3, 128 filters → 16×16×128
MaxPool 2×2 → 8×8×128
Flatten → 8×8×128 = 8,192
FC → 128 (embedding dimension)

Output: 128-dimensional embedding vector
```

**Forward Pass:**

```
Image 1 → Network → Embedding 1: [0.12, 0.45, 0.67, 0.23, ...]  (128 values)
Image 2 → Network → Embedding 2: [0.15, 0.43, 0.69, 0.21, ...]  (128 values)

Compute L2 distance:
d = √(Σᵢ(e1ᵢ - e2ᵢ)²)
  = √((0.12-0.15)² + (0.45-0.43)² + (0.67-0.69)² + ...)
  = √(0.0009 + 0.0004 + 0.0004 + ... (128 terms))
  = √0.0856
  = 0.293

Decision:
If threshold = 0.5:
  d = 0.293 < 0.5 → "SAME person" ✓
```

### Example 2: Computing Embeddings

**Step-by-Step Through Network:**

**Input Image:**
```
Person face (64×64×3)
Represented as matrix of pixel values [0-255]
Normalized to [0-1] range
```

**After Conv1:**
```
64×64×3 → Conv 3×3, 32 filters → 64×64×32

Each of 32 channels detects different features:
Channel 0: Edge detection
Channel 1: Texture detection
Channel 5: Nose feature
Channel 12: Eye feature
(learned automatically during training)
```

**After Pooling and More Convs:**
```
→ 32×32×32 → 16×16×64 → 8×8×128

Final features: 8×8×128 = 8,192 values
Rich representation of face features
```

**Final Embedding:**
```
8,192 values → FC layer → 128 dimensions

This compression captures the essential face identity:
[0.12, 0.45, 0.67, 0.23, -0.15, 0.89, ...]

Each dimension captures some aspect of face identity
Combination uniquely represents this person
```

### Example 3: Distance Calculation

**Complete Example with Real Numbers:**

**Person A - Photo 1:**
```
Embedding_A1 = [0.50, 0.30, 0.80, 0.20, 0.60] (simplified to 5D)
```

**Person A - Photo 2:**
```
Embedding_A2 = [0.52, 0.28, 0.82, 0.19, 0.61]
```

**Person B - Photo 1:**
```
Embedding_B1 = [0.10, 0.90, 0.25, 0.85, 0.15]
```

**Calculations:**

```
Distance(A1, A2) - Same person:
Differences: [0.02, 0.02, 0.02, 0.01, 0.01]
Squared: [0.0004, 0.0004, 0.0004, 0.0001, 0.0001]
Sum: 0.0014
d(A1,A2) = √0.0014 = 0.037

Distance(A1, B1) - Different people:
Differences: [0.40, 0.60, 0.55, 0.65, 0.45]
Squared: [0.16, 0.36, 0.3025, 0.4225, 0.2025]
Sum: 1.4475
d(A1,B1) = √1.4475 = 1.203

Comparison:
d(A1,A2) = 0.037 << threshold (0.5) → Same person ✓
d(A1,B1) = 1.203 >> threshold (0.5) → Different people ✓

The network successfully learned to distinguish!
```

### Example 4: Complete Face Verification

**Scenario:** Security system verifying employee ID

**Database:**
```
Employee 1: stored_embedding_1 = [0.2, 0.7, 0.3, ...]
Employee 2: stored_embedding_2 = [0.8, 0.1, 0.9, ...]
...
Employee 100: stored_embedding_100 = [0.5, 0.5, 0.4, ...]
```

**Verification Process:**

```
Step 1: Capture photo of person at door
photo = camera_image (64×64×3)

Step 2: Compute embedding
embedding_new = Siamese_Network(photo)
              = [0.21, 0.68, 0.32, ...]

Step 3: Compare with all database embeddings
d₁ = ||embedding_new - stored_embedding_1|| = 0.045
d₂ = ||embedding_new - stored_embedding_2|| = 1.23
d₃ = ||embedding_new - stored_embedding_3|| = 0.87
...
d₁₀₀ = ||embedding_new - stored_embedding_100|| = 0.75

Step 4: Find minimum distance
min_distance = min(d₁, d₂, ..., d₁₀₀) = d₁ = 0.045
matched_employee = Employee 1

Step 5: Decision
If min_distance < threshold (0.5):
   "Access granted: Employee 1"
Else:
   "Access denied: Unknown person"

Result: Granted (0.045 < 0.5) - correct match!
```

---

## Forward Propagation

### Processing Image Pairs

**Complete Forward Pass Algorithm:**

```
Algorithm: Siamese Network Forward Pass

Input:
  - image1: First image (H × W × 3)
  - image2: Second image (H × W × 3)
  - encoder: Shared network f(x; θ)

Steps:

1. Process first image through encoder:
   embedding1 = encoder(image1)
   Shape: (d,) where d = embedding dimension
   
2. Process second image through SAME encoder:
   embedding2 = encoder(image2)
   Shape: (d,)
   
   Note: Same weights θ used for both!

3. Compute distance:
   distance = ||embedding1 - embedding2||₂
   
   Or compute similarity:
   similarity = cosine_similarity(embedding1, embedding2)

4. Make decision:
   If distance < threshold:
      prediction = "SAME person"
   Else:
      prediction = "DIFFERENT people"

5. Return (distance, prediction) or (similarity, prediction)
```

### Computing Embeddings

**Step-by-Step:**

```
Image: 64×64×3 (RGB face photo)

Layer 1: Conv 3×3, 32 filters, ReLU
Output: 64×64×32

Layer 2: MaxPool 2×2, stride=2
Output: 32×32×32

Layer 3: Conv 3×3, 64 filters, ReLU
Output: 32×32×64

Layer 4: MaxPool 2×2, stride=2
Output: 16×16×64

Layer 5: Conv 3×3, 128 filters, ReLU
Output: 16×16×128

Layer 6: MaxPool 2×2, stride=2
Output: 8×8×128

Layer 7: Flatten
Output: 8,192 values

Layer 8: FC to 128 dimensions, L2 normalization
Output: 128-dimensional embedding (unit norm)

Final embedding: [e₁, e₂, ..., e₁₂₈]
Where ||embedding||₂ = 1 (normalized)
```

### Distance Computation

**L2 Distance Implementation:**

```
Embedding 1: e₁ = [e₁₁, e₁₂, ..., e₁₁₂₈]
Embedding 2: e₂ = [e₂₁, e₂₂, ..., e₂₁₂₈]

Step 1: Compute differences
diff = e₁ - e₂ = [e₁₁-e₂₁, e₁₂-e₂₂, ..., e₁₁₂₈-e₂₁₂₈]

Step 2: Square each difference
diff_squared = [diff₁², diff₂², ..., diff₁₂₈²]

Step 3: Sum
sum_squares = Σᵢ diff_i²

Step 4: Square root
distance = √sum_squares
```

### Numerical Example

**Concrete Numbers:**

**Image 1 (Person A, frontal):**

After forward pass:
```
embedding1 = [0.12, -0.34, 0.67, 0.45, -0.23, 0.89, ..., 0.15]
             (128 values, normalized to unit length)
```

**Image 2 (Person A, profile):**

After forward pass:
```
embedding2 = [0.15, -0.32, 0.65, 0.43, -0.25, 0.91, ..., 0.13]
```

**Distance Calculation:**

```
diff = [0.12-0.15, -0.34-(-0.32), 0.67-0.65, 0.45-0.43, ...]
     = [-0.03, -0.02, 0.02, 0.02, ...]

diff² = [0.0009, 0.0004, 0.0004, 0.0004, ...]

sum = 0.0009 + 0.0004 + 0.0004 + ... (128 terms) = 0.152

distance = √0.152 = 0.39
```

**Decision:**

```
Threshold = 0.5

distance (0.39) < threshold (0.5)
→ Prediction: "SAME person"
→ Confidence: 1 - (0.39/0.5) = 0.22 or 22% margin below threshold
```

---

## Backward Propagation

### Gradient Flow Through Twin Networks

**Plain English:**

During backprop, gradients flow backward through both networks. Because the networks share weights, gradients from both paths combine when updating the weights.

**Visual Gradient Flow:**

```
FORWARD:
Image 1 → Network (θ) → Embedding 1 ─┐
                                      ├→ Distance → Loss
Image 2 → Network (θ) → Embedding 2 ─┘

BACKWARD:
         ∂L/∂θ ← Network ← ∂L/∂emb1 ←┐
                                      ├─ ∂L/∂distance
         ∂L/∂θ ← Network ← ∂L/∂emb2 ←┘
              ↓
        Gradients from both images
        combine because weights shared!
        
Final gradient: ∂L/∂θ = ∂L/∂θ(from img1) + ∂L/∂θ(from img2)
```

### Weight Sharing Implications

**Mathematical Impact:**

```
Loss depends on both embeddings:
L = L(f(x₁; θ), f(x₂; θ))

Taking derivative with respect to θ:
∂L/∂θ = ∂L/∂f(x₁) × ∂f(x₁)/∂θ + ∂L/∂f(x₂) × ∂f(x₂)/∂θ

Two gradient contributions:
1. From processing image 1
2. From processing image 2

Both update the same weights θ!
```

**Why This Helps:**

```
Weight sharing means:
- Each parameter gets gradients from multiple images per pair
- Faster learning (more gradient information per update)
- Ensures consistency (same face always maps similarly)
- Reduces total parameters (one network, not two)
```

### Gradient Computation for Triplet Loss

**Triplet Loss:**

```
L = max(d(A,P) - d(A,N) + margin, 0)

Where:
d(A,P) = ||f(A) - f(P)||²
d(A,N) = ||f(A) - f(N)||²
```

**Gradient with Respect to Embeddings:**

```
If loss > 0 (margin violated):

∂L/∂f(A) = 2 × [(f(P) - f(A)) - (f(N) - f(A))]
         = 2 × [f(P) - f(N)]

∂L/∂f(P) = 2 × [f(A) - f(P)]

∂L/∂f(N) = 2 × [f(N) - f(A)]

If loss = 0 (margin satisfied):
All gradients = 0 (no update needed)
```

**Interpretation:**

```
∂L/∂f(A): Pushes anchor toward positive, away from negative
∂L/∂f(P): Pulls positive closer to anchor
∂L/∂f(N): Pushes negative farther from anchor

Visual in embedding space:
     N              A    P
     ×   ←───   ×──→×
     
Anchor moves toward positive, away from negative
Positive pulled toward anchor
Negative pushed away from anchor
```

### Detailed Gradient Example

**Given Triplet:**

```
Anchor embedding: A = [0.5, 0.8, 0.3]
Positive embedding: P = [0.52, 0.79, 0.31]
Negative embedding: N = [0.6, 0.2, 0.7]

Distances:
d(A,P) = ||(0.5-0.52, 0.8-0.79, 0.3-0.31)||
       = ||(-0.02, 0.01, -0.01)||
       = √(0.0004 + 0.0001 + 0.0001)
       = √0.0006 = 0.024

d(A,N) = ||(0.5-0.6, 0.8-0.2, 0.3-0.7)||
       = ||(-0.1, 0.6, -0.4)||
       = √(0.01 + 0.36 + 0.16)
       = √0.53 = 0.728

Margin = 0.2
```

**Loss Calculation:**

```
L = max(d(A,P) - d(A,N) + margin, 0)
  = max(0.024 - 0.728 + 0.2, 0)
  = max(-0.504, 0)
  = 0

Margin satisfied! No update needed.

But for demonstration, let's compute gradients anyway:
```

**Gradients (if loss > 0):**

```
∂L/∂A = 2 × [P - N]
      = 2 × [(0.52, 0.79, 0.31) - (0.6, 0.2, 0.7)]
      = 2 × [(-0.08, 0.59, -0.39)]
      = [-0.16, 1.18, -0.78]

∂L/∂P = 2 × [A - P]
      = 2 × [(0.5, 0.8, 0.3) - (0.52, 0.79, 0.31)]
      = 2 × [(-0.02, 0.01, -0.01)]
      = [-0.04, 0.02, -0.02]

∂L/∂N = 2 × [N - A]
      = 2 × [(0.6, 0.2, 0.7) - (0.5, 0.8, 0.3)]
      = 2 × [(0.1, -0.6, 0.4)]
      = [0.2, -1.2, 0.8]
```

**Backprop to Network Weights:**

```
These embedding gradients flow back through the network:

∂L/∂θ = ∂L/∂A × ∂A/∂θ + ∂L/∂P × ∂P/∂θ + ∂L/∂N × ∂N/∂θ

Three gradient contributions (one from each image in triplet)
All update the same shared weights θ
```

---

## Summary

**One-Shot Learning:**
- Recognize from single example
- Critical for face recognition (can't have 1000 photos per person)
- Learns similarity metric, not classification

**Siamese Networks:**
- Twin networks with shared weights
- Transform images to embeddings
- Compare embeddings to measure similarity
- Enables one-shot learning

**Key Components:**
- Shared encoder: Produces consistent embeddings
- Embedding space: Where similar faces are close
- Distance metric: Measures similarity (L2, L1, cosine)
- Loss function: Contrastive or triplet loss

**Training:**
- Pairs or triplets of images
- Push same-person embeddings together
- Push different-person embeddings apart
- Hard negative mining for efficiency

**Forward Pass:**
- Image → Encoder → Embedding
- Compare embeddings → Distance
- Threshold → Same/Different decision

**Backward Pass:**
- Gradients from both/all images
- Combined due to weight sharing
- Updates improve similarity metric

**Advantages:**
- Works with one example per person
- Scales to millions of people
- No retraining when adding people
- Just store new person's embedding

This makes Siamese networks ideal for face recognition, signature verification, and any task requiring comparison of examples!