...
3. **Poor Interpretation for Small Angles**:
   ```
   For small angles θ (similar words):
   sin(θ) ≈ θ (in radians)
   ...
   ✓ Cosine provides better discrimination for similar words
   ```
...
##### Numerical Example: Comparing All Metrics

Let's compute different metrics for the same word pairs to see why cosine and Euclidean are preferred:

```
Given three word embeddings (2D for simplicity):
═════════════════════════════════════════════════

"cat"    = [0.6, 0.8]  (magnitude = 1.0)
"kitten" = [0.48, 0.64] (magnitude = 0.8, same direction as cat)
"car"    = [0.8, 0.6]  (magnitude = 1.0, different direction)


Example 1: Comparing "cat" vs "kitten" (similar meaning, different magnitude)
═══════════════════════════════════════════════════════════════════════════════

Step 1: Cosine Similarity
─────────────────────────

cos(θ) = (cat · kitten) / (||cat|| × ||kitten||)
       = (0.6×0.48 + 0.8×0.64) / (1.0 × 0.8)
       = (0.288 + 0.512) / 0.8
       = 0.800 / 0.8
       = 1.0 ✓

Interpretation: Perfect semantic match (ignores that "kitten" is rarer)
...
Interpretation: Small distance (close in space)


Step 3: Sine (angle between vectors)
─────────────────────────────────────

Since cos(θ) = 1.0, we have θ = 0°
sin(0°) = 0 ✗

Interpretation: Zero similarity? Wrong! They're identical in meaning.


Step 4: Dot Product (unnormalized)
───────────────────────────────────

cat · kitten = 0.6×0.48 + 0.8×0.64
             = 0.288 + 0.512
             = 0.800

Interpretation: Affected by magnitudes, but still captures similarity


Example 2: Comparing "cat" vs "car" (different meaning, similar magnitude)
═══════════════════════════════════════════════════════════════════════════

Step 1: Cosine Similarity
─────────────────────────

cos(θ) = (cat · car) / (||cat|| × ||car||)
       = (0.6×0.8 + 0.8×0.6) / (1.0 × 1.0)
       = (0.48 + 0.48) / 1.0
       = 0.96

Wait, let me recalculate the angle more precisely:

cat = [0.6, 0.8], car = [0.8, 0.6]
These vectors make an angle. Let's compute:

cat · car = 0.48 + 0.48 = 0.96
||cat|| = 1.0, ||car|| = 1.0
cos(θ) = 0.96

θ = arccos(0.96) ≈ 16.3°

This is quite similar actually. Let me use MORE different vectors:

Revised: "car" = [0.8, -0.6] (magnitude = 1.0, very different direction)

cos(θ) = (0.6×0.8 + 0.8×(-0.6)) / (1.0 × 1.0)
       = (0.48 - 0.48) / 1.0
       = 0.0 ✓

Interpretation: Orthogonal = completely unrelated semantics
...
Interpretation: Large distance = different
...
Interpretation: Maximum sine. But what does this mean?
- High sine value suggests... difference? But sine is also high at 45°!
...
Interpretation: Cannot compute! Metric breaks down.


Example 3: Comparing "cat" vs "-cat" (opposite direction)
══════════════════════════════════════════════════════════

"cat"  = [0.6, 0.8]
"-cat" = [-0.6, -0.8] (complete opposite, like antonym)

Step 1: Cosine Similarity
─────────────────────────

cos(θ) = (0.6×(-0.6) + 0.8×(-0.8)) / (1.0 × 1.0)
       = (-0.36 - 0.64) / 1.0
       = -1.0 ✓

Interpretation: Opposite meaning (antonyms)
...
Interpretation: Maximum distance for unit vectors
...
Interpretation: Zero, same as identical vectors! Cannot distinguish.
...
Interpretation: Zero, same as identical vectors! Useless.


Summary Table:
══════════════

                        cat vs kitten    cat vs car      cat vs -cat
                        (same dir,       (orthogonal)    (opposite)
                         diff magnitude)
                        ─────────────    ────────────    ────────────
Cosine Similarity       1.0    ✓         0.0    ✓        -1.0   ✓
                        (identical)      (unrelated)     (opposite)

Euclidean Distance      0.2    ✓         1.414  ✓        2.0    ✓
                        (close)          (far)           (farthest)

Sine                    0.0    ✗         1.0    ?        0.0    ✗
                        (ambiguous)      (ambiguous)     (same as identical!)

Tangent                 0.0    ✗         ∞      ✗        0.0    ✗
                        (ambiguous)      (undefined!)    (ambiguous)

Dot Product             0.8    ⚠         0.0    ✓        -1.0   ✓
(unnormalized)          (affected by     (unrelated)     (opposite)
                         magnitude)

✓ = Useful, ✗ = Problematic, ⚠ = Depends on context
```
...
Task                          Metric              Why
────────────────────────────────────────────────────────────────
Word similarity               Cosine              Meaning, not frequency
Sentence similarity           Cosine              Same as word similarity
Document retrieval            Cosine              Standard in IR
K-means clustering            Euclidean           Algorithm requirement
Finding k nearest neighbors   Euclidean or Cosine Depends on normalization
Attention mechanisms          Dot product         Efficient, learnable scale
Measuring vector angles       Cosine              Direct interpretation
                             (arccos → angle)
...
From the embedding matrix E, column 2 (index 2):
...
The one-hot vector multiplication simply selects the corresponding column.
```

#### ASCII Visualization of Lookup Process

```
╔═══════════════════════════════════════════════════════════════════════╗
║               COMPLETE EMBEDDING LOOKUP VISUALIZATION                 ║
╚═══════════════════════════════════════════════════════════════════════╝
...
│   ════════════════════════
│   
│   Index 2 → One-hot vector (V = 5 dimensions)
│   
│         Position: 0   1   2   3   4
│                   │   │   │   │   │
│   o_good      =  [0,  0,  1,  0,  0]
│                         ↑
│                         └─ Only position 2 is "hot" (= 1)
│
├─► Step 3: Matrix Multiplication
│   ══════════════════════════════
│   
...
└─► Step 4: Extract Result
    ...
    e_good = [0.72]  ← dim 0 value for "good"
             [0.43]  ← dim 1 value for "good"
             [0.19]  ← dim 2 value for "good"
    
    This is the 3-dimensional embedding vector for "good"!


Alternative View: Direct Column Extraction
═══════════════════════════════════════════
...
Key Insight:
═══════════
In practice, we don't actually do the matrix multiplication!
We simply INDEX into the matrix to get the column directly:
  e_good = E[:, 2]  (much faster!)

The matrix multiplication view is useful for understanding gradients
