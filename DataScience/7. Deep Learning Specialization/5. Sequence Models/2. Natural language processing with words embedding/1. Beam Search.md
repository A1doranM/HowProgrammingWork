...
**Exhaustive Search** evaluates all possible sequences:

$$y^* = \underset{y}{\text{argmax}} \; P(y | x)$$

**Computational Complexity**: $O(|V|^T)$ - exponential in sequence length

**Concrete Infeasibility**:
```
╔═══════════════════════════════════════════════════════════════════╗
║         EXHAUSTIVE SEARCH COMPUTATIONAL REQUIREMENTS              ║
╚═══════════════════════════════════════════════════════════════════╝

Scenario: English text generation
├── Vocabulary: |V| = 50,000 words
├── Sequence length: T = 20 words
└── Total sequences: 50,000^20 ≈ 10^94

Assumptions:
├── Modern GPU: 10^12 sequences/second (generous overestimate)
└── Time to evaluate all: 10^94 / 10^12 = 10^82 seconds

Comparison:
├── Age of universe: ~10^17 seconds
├── Time needed: 10^82 seconds = 10^65 times the age of the universe!
└── Conclusion: IMPOSSIBLE even with all computers on Earth

Memory Requirements:
├── Each sequence: ~100 bytes
├── Total memory: 10^94 × 100 = 10^96 bytes
└── Observable universe atoms: ~10^80 atoms (not enough to store it!)
```
...
def beam_search(model, x, B, V, max_length):
    # ─────────────────────────────────────────────────────────
    # STEP 1: INITIALIZATION
    # ─────────────────────────────────────────────────────────
    # Start with single hypothesis containing <START> token
    active_hypotheses = [
        {
            'tokens': ['<START>'],
            'score': 0.0,           # Log probability sum
            'length': 1
        }
    ]
    ...
    # ─────────────────────────────────────────────────────────
    # STEP 2: MAIN LOOP - Generate one token at a time
    # ─────────────────────────────────────────────────────────
    for t in range(1, max_length + 1):
        all_candidates = []
        
        # ─────────────────────────────────────────────────────
        # STEP 2a: EXPANSION - Expand each active hypothesis
        # ─────────────────────────────────────────────────────
        for hyp in active_hypotheses:
            # Get probability distribution for next token
            # Shape: |V| probabilities
            probs = model.predict_next(x, hyp['tokens'])
            
            # Generate B × |V| candidates
            for token in V:
                # Compute new hypothesis
                new_hyp = {
                    'tokens': hyp['tokens'] + [token],
                    'score': hyp['score'] + log(probs[token]),
                    'length': hyp['length'] + 1
                }
                
                # Check if sequence is complete
                if token == '<EOS>':
                    new_hyp['complete'] = True
                    completed_hypotheses.append(new_hyp)
                else:
                    new_hyp['complete'] = False
                    all_candidates.append(new_hyp)
        
        # ─────────────────────────────────────────────────────
        # STEP 2b: TERMINATION CHECK
        # ─────────────────────────────────────────────────────
        # If no active candidates remain, all beams finished
        if len(all_candidates) == 0:
            break
        
        # ─────────────────────────────────────────────────────
        # STEP 2c: PRUNING - Keep only top-B hypotheses
        # ─────────────────────────────────────────────────────
        # Sort by score (descending - higher is better)
        all_candidates.sort(key=lambda h: h['score'], reverse=True)
        
        # Keep only top-B
        active_hypotheses = all_candidates[:B]
        
        # Optional: Early stopping
        if len(completed_hypotheses) > 0:
            best_completed_score = max(h['score'] for h in completed_hypotheses)
            best_active_score = active_hypotheses[0]['score']
            
            # If best active can't beat best completed, stop
    ...
    # ─────────────────────────────────────────────────────────
    # Add any remaining active hypotheses to completed
    for hyp in active_hypotheses:
        hyp['complete'] = True
        completed_hypotheses.append(hyp)
    
    # Select best hypothesis (highest score)
    # Note: May apply length normalization here
    best_hypothesis = max(completed_hypotheses, key=lambda h: h['score'])
    
    return best_hypothesis['tokens']
...
# ═══════════════════════════════════════════════════════════
"""
Time Complexity: O(B × T × |V|)
  - T iterations (sequence length)
  - Each iteration: Expand B hypotheses
  - Each expansion: Consider |V| tokens
  - Sorting: O(B × |V| × log(B × |V|)) ≈ O(B × |V| × log|V|)
  - Total: O(T × B × |V| × log|V|)

Space Complexity: O(B × T)
  - Store B hypotheses
  - Each hypothesis has length ≤ T
  
Comparison:
  - Greedy:     O(T × |V|)           [B = 1]
  - Beam (B=5): O(5 × T × |V|)       [5x greedy]
  - Exhaustive: O(|V|^T)             [exponential!]
"""
```

### Visual Walkthrough

#### Complete Beam Search Tree Visualization

```
╔═══════════════════════════════════════════════════════════════════════╗
║           BEAM SEARCH TREE - COMPLETE WALKTHROUGH                     ║
║                                                                       ║
║  Task: Machine Translation "Je suis" → English                       ║
║  Vocabulary: {I, am, a, we, are, <EOS>}  (|V| = 6)                  ║
║  Beam Width: B = 2                                                   ║
╚═══════════════════════════════════════════════════════════════════════╝

Legend:
  ■ = Active beam (being expanded)
  □ = Pruned hypothesis (discarded)
  ● = Completed sequence (<EOS> generated)
  ─ = Expansion edge
  ═ = Final selected path
...
Expand <START> to all 6 possible first tokens:
        ...
        │      │       │       │       │       │      │
      [I]    [am]    [a]     [we]   [are]   [<EOS>]
      -0.3   -2.5    -1.8    -0.8   -3.0     -5.0
       ■      □       □       ■      □        □
       │                      │
      KEEP                   KEEP    ← Top-2: "I" and "we"
       │                      │
       └──────────────────────┘
              │
     Pruned: [am], [a], [are], [<EOS>] (4 hypotheses discarded)
...
Expand 2 active beams × 6 tokens = 12 candidates:
...
─────────────                      ─────────────
  ...
  ┌───┼───┬───┬───┬────┬─────        ┌───┼───┬───┬───┬────┬─────
  │   │   │   │   │    │             │   │   │   │   │    │
 [I] [am][a][we][are][<EOS>]        [I] [am][a][we][are][<EOS>]
  │   │   │   │   │    │             │   │   │   │   │    │
-2.0-0.9-1.5-2.8-3.1 -4.0          -2.5-1.5-1.9-2.9-0.9  -3.5
  □   ■   □   □   □    □             □   □   □   □   ■    □

Scores (combining with previous):
  From "I":
    ["I", "I"]:    -0.3 + (-1.7) = -2.0  □ Pruned
    ["I", "am"]:   -0.3 + (-0.6) = -0.9  ■ KEEP (rank 1)
    ["I", "a"]:    -0.3 + (-1.2) = -1.5  □ Pruned
    ["I", "we"]:   -0.3 + (-2.5) = -2.8  □ Pruned
    ["I", "are"]:  -0.3 + (-2.8) = -3.1  □ Pruned
    ["I", "<EOS>"]: -0.3 + (-3.7) = -4.0  □ Pruned (unlikely to end here)
    
  From "we":
    ["we", "I"]:    -0.8 + (-1.7) = -2.5  □ Pruned
    ["we", "am"]:   -0.8 + (-0.7) = -1.5  □ Pruned
    ["we", "a"]:    -0.8 + (-1.1) = -1.9  □ Pruned
    ["we", "we"]:   -0.8 + (-2.1) = -2.9  □ Pruned
    ["we", "are"]:  -0.8 + (-0.1) = -0.9  ■ KEEP (rank 2)
    ["we", "<EOS>"]: -0.8 + (-2.7) = -3.5  □ Pruned

Ranking all 12 candidates:
  1. ["I", "am"]:   -0.9  ■ KEEP
  2. ["we", "are"]: -0.9  ■ KEEP
  3. ["I", "a"]:    -1.5  □ Prune
  4. ["we", "am"]:  -1.5  □ Prune
...
Expand 2 beams × 6 tokens = 12 candidates:

From ["I", "am"]:                  From ["we", "are"]:
──────────────────                 ───────────────────
 ...
 [I]  [am] [a][we][are][<EOS>]       [I]  [am] [a][we][are][<EOS>]
  │     │    │   │   │    │           │     │    │   │   │    │
-3.5 -3.2 -1.2-3.8-4.0  -0.5        -3.1 -3.4 -1.8-3.5-4.2  -0.3
  □     □    □   □   □    ●           □     □    □   □   □    ●

Scores:
  From ["I", "am"]:
    ["I", "am", "I"]:      -0.9 + (-2.6) = -3.5  □ Pruned
    ["I", "am", "am"]:     -0.9 + (-2.3) = -3.2  □ Pruned
    ["I", "am", "a"]:      -0.9 + (-0.3) = -1.2  ■ POTENTIAL (but will be pruned)
    ["I", "am", "we"]:     -0.9 + (-2.9) = -3.8  □ Pruned
    ["I", "am", "are"]:    -0.9 + (-3.1) = -4.0  □ Pruned
    ["I", "am", "<EOS>"]:  -0.9 + ( 0.4) = -0.5  ● COMPLETE (rank 2)
    
  From ["we", "are"]:
    ["we", "are", "I"]:     -0.9 + (-2.2) = -3.1  □ Pruned
    ["we", "are", "am"]:    -0.9 + (-2.5) = -3.4  □ Pruned
    ["we", "are", "a"]:     -0.9 + (-0.9) = -1.8  □ Pruned
    ["we", "are", "we"]:    -0.9 + (-2.6) = -3.5  □ Pruned
    ["we", "are", "are"]:   -0.9 + (-3.3) = -4.2  □ Pruned
    ["we", "are", "<EOS>"]: -0.9 + ( 0.6) = -0.3  ● COMPLETE (rank 1)

Completed Sequences (contain <EOS>):
  1. ["we", "are", "<EOS>"]  score: -0.3  ● BEST
  2. ["I", "am", "<EOS>"]    score: -0.5  ●

Active Beams (not yet complete):
  1. ["I", "am", "a"]        score: -1.2  (but worse than completed)
  2. ["we", "are", "a"]      score: -1.8  (but worse than completed)

EARLY STOPPING: Best active (-1.2) < Best completed (-0.3)
→ Continuing can't improve the result
→ TERMINATE
...
┌────────────────────────────────────────┐
│ 1. ["we", "are", "<EOS>"]  -0.3  ★★★  │ ← BEST SCORE
│ 2. ["I", "am", "<EOS>"]    -0.5  ★★   │
└────────────────────────────────────────┘

Selected Output: "we are"


═══════════════════════════════════════════════════════════════════════
PATH VISUALIZATION - FULL TREE WITH FINAL PATH
═══════════════════════════════════════════════════════════════════════
    ...
    └───────┘  └───┬───┘  └─────── └──────┘└──╩═══╝
        ...
        └──────┴──────┴──────┘      └──────┘   └══════┘
           □      □      □             □           ●
         Pruned Pruned Pruned       Pruned    SELECTED!

Legend:
  ═══ Selected path (double lines)
