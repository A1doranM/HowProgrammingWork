...
- [13. The Mathematics of EWA](#2-the-mathematics)
  ...
  - [What β Controls](#what-does-β-control)
- [14. Complete Numerical Examples](#3-complete-numerical-example-temperature-data)
  - [Temperature Data Setup](#setup)
  - [Case 1: β = 0.9](#case-1-β--09-fast-adaptation)
  - [Case 2: β = 0.98](#case-2-β--098-slow-adaptation)
...
- [17. Comparing Different β Values](#4-comparing-different-β-values)
  - [Multiple β Analysis](#temperature-data-with-multiple-β)
...
- [28. Complete Numerical Example](#3-complete-numerical-example)
  ...
  - [With Momentum](#with-momentum-β--09)
...
- [30. Complete Cat vs Dog Example](#5-complete-example-cat-vs-dog-network)
  ...
  - [Training With Momentum](#training-with-momentum-β--09-1)
...
- [32. Two Formulations](#7-the-two-formulations-of-momentum)
  - [Classic vs Modern](#formulation-1-classic-with-1-β-factor)
...
- [34. Comparing Momentum Values](#9-comparing-different-momentum-values)
  - [Different β Analysis](#cat-vs-dog-network-same-data)
...
- [37. Practical Guidelines for Momentum](#12-practical-guidelines-for-momentum)
  - [Choosing β](#choosing-β-momentum-coefficient)
...
- [43. Step-by-Step Numerical Example](#3-step-by-step-numerical-example)
  ...
  - [Realistic Examples](#realistic-example-with-β--0999)
...
Total parameters: 1000×100 + 100×2 = 100,200 weights
```
...
∂L/∂w = (∂Loss₁/∂w + ∂Loss₂/∂w + ... + ∂Loss₁₂₈/∂w) / 128

Example for weight W[50, 30] (connecting feature 30 to hidden neuron 50):

Image 1: ∂Loss₁/∂W[50,30] = 0.023
Image 2: ∂Loss₂/∂W[50,30] = -0.018
Image 3: ∂Loss₃/∂W[50,30] = 0.031
...
Image 128: ∂Loss₁₂₈/∂W[50,30] = -0.015

Average gradient:
∂L/∂W[50,30] = (0.023 - 0.018 + 0.031 + ... - 0.015) / 128
              = 0.812 / 128
              = 0.00634
...
```
Image 1 (cat):
x₁ = [0.12, 0.45, 0.89, 0.23, ...]  (1000 features)

z₁ = W×x₁ + b
ŷ₁ = softmax(z₁) = [0.52, 0.48]
y₁ = [1, 0]
Loss₁ = -log(0.52) = 0.654
```
...
Standard deviation: 0.0892  ← Much more noisy!
Min gradient: -0.3145
Max gradient: 0.2891
...
**Gradient:**
```
∂Loss₂/∂W[50,30] = -0.018  (different sign from before!)
```

**Weight Update:**

```
W[50,30] := 0.2477 - 0.1 × (-0.018)
         := 0.2477 + 0.0018
         := 0.2495
...
Iteration 1 (image 1):  W = 0.2477, Loss₁ = 0.654
Iteration 2 (image 2):  W = 0.2495, Loss₂ = 0.598
Iteration 3 (image 3):  W = 0.2464, Loss₃ = 0.494
...
Iteration 128 (image 128): W = 0.2493, Loss₁₂₈ = 0.478
...
```
Image 1: ŷ₁ = [0.52, 0.48], Loss₁ = 0.654
Image 2: ŷ₂ = [0.45, 0.55], Loss₂ = 0.598
Image 3: ŷ₃ = [0.61, 0.39], Loss₃ = 0.494
...
Image 32: ŷ₃₂ = [0.58, 0.42], Loss₃₂ = 0.543

Average loss for this mini-batch:
L_mini = (Loss₁ + Loss₂ + ... + Loss₃₂) / 32
...
Average gradient:
∂L_mini/∂W[50,30] = (0.023 - 0.018 + 0.031 + ... + 0.019) / 32
                   = 0.197 / 32
                   = 0.00616
...
Standard deviation: 0.0156  ← Between SGD and Batch!
Min gradient: -0.0847
Max gradient: 0.0923
...
```
Average loss: 0.571
∂L_mini/∂W[50,30] = 0.00682

Update:
W[50,30] := 0.249384 - 0.1 × 0.00682
         := 0.248702
```
...
```
Average loss: 0.548
∂L_mini/∂W[50,30] = 0.00591

Update:
W[50,30] := 0.248702 - 0.1 × 0.00591
         := 0.248111
```
...
```
Average loss: 0.563
∂L_mini/∂W[50,30] = 0.00658

Update:
W[50,30] := 0.248111 - 0.1 × 0.00658
         := 0.247453
```
...
0.250 ──→ 0.249384 ──→ 0.248702 ──→ 0.248111 ──→ 0.247453
      ↑                                              ↑
...
```
  ...
  0.7│
  0.6│●                           Batch GD
  0.5│ ╲___________________________
     │
  0.7│●╲  ╱╲  ╱╲  ╱╲              SGD
  0.6│  ╲╱  ╲╱  ╲╱  ╲_╱╲_
  0.5│                    ╲___
...
| **Mini-batch** | 32 | 4 | 1.2s | Medium (32 images) | Medium | **Optimal** ✓ |
...
Larger batch size → Lower variance
...
Cons:
✗ Noisy gradients
✗ Can't exploit GPU parallelism well
✗ Unstable training
...
Cons:
✗ Slower updates (fewer per epoch)
...
|-----------|---------------|---------------|-----------|----------------|---------------|-------|
| **1** (SGD) | 1024 | 10,240 | 8.5s | 98% | 88% | ✗ Slow |
| **8** | 128 | 1,280 | 3.2s | 97% | 91% | Good |
| **16** | 64 | 640 | 2.1s | 96% | 92% | Good |
| **32** | 32 | 320 | 1.5s | 95% | **93%** | ✓ Best! |
| **64** | 16 | 160 | 1.2s | 94% | 92% | Good |
| **128** | 8 | 80 | 1.0s | 93% | 91% | Good |
| **256** | 4 | 40 | 0.9s | 91% | 88% | ✗ Underfit |
| **1024** (Batch) | 1 | 10 | 0.8s | 85% | 82% | ✗ Underfit |
...
New learning rate = 0.1 × (128/32) = 0.1 × 4 = 0.4

Why? Larger batches → smoother gradients → can take bigger steps
```
...
```
Batch size 32:
μ_B = 0.05, σ_B = 1.02  (good estimate)

Batch size 2:
μ_B = 0.23, σ_B = 0.67  (noisy estimate!)

Batch size 1:
μ_B = x_1, σ_B = 0  (undefined!)
Can't normalize!
```

**Guideline:** If using Batch Norm, use batch size ≥ 16, preferably ≥ 32
...
├─ YES → Increase batch size (64 or 128)
...
│         └─ High utilization? → Keep current size
│
└─ NO → Continue

Is test accuracy much lower than train?
├─ YES → Might be overfitting
│         Try DECREASING batch size (16 or 8)
│         Smaller batches = better generalization
│
└─ NO → You're good! ✓

Using Batch Normalization?
├─ YES → Keep batch size ≥ 16, prefer ≥ 32
└─ NO → Batch size 8-16 is fine

Memory limited?
├─ YES → Use smallest batch size that fits
│         Use gradient accumulation to simulate larger
└─ NO → Enjoy the freedom!
```
...
**Typical results:**
```
Batch size 16:  Optimal LR ≈ 0.01
Batch size 32:  Optimal LR ≈ 0.02
Batch size 64:  Optimal LR ≈ 0.04
Batch size 128: Optimal LR ≈ 0.08
...
### **❌ Mistake 1: Using batch size that doesn't divide dataset size**
...
### **❌ Mistake 3: Forgetting to shuffle**

```python
# BAD: No shuffling
train_loader = DataLoader(dataset, batch_size=32, shuffle=False)
# Model sees images in same order every epoch!
# First batch always cats, second always dogs → poor learning

# GOOD: Shuffle each epoch
train_loader = DataLoader(dataset, batch_size=32, shuffle=True)
# Different mini-batches each epoch → better generalization
```

---

### **❌ Mistake 4: Different batch sizes for train and validation**
...
│   Use: Small datasets, when stability critical     │
│                                                      │
│ Mini-Batch GD: ✓ RECOMMENDED                       │
│   Batch Size = 16-128 (typically 32)              │
│   Updates/Epoch = N/B                              │
│   Pros: Balanced speed/stability, GPU efficient    │
...
│   Batch Size = 1                                   │
│   Updates/Epoch = N                                │
│   Pros: Fast per update, regularization effect    │
│   Cons: Very noisy, unstable, slow overall        │
│   Use: Rarely alone, mostly for theory            │
│                                                      │
└─────────────────────────────────────────────────────┘
```
...
│                                          │
│ NLP/Sequences: 16-64                     │
│   Variable lengths need more memory      │
│                                          │
│ Small Dataset (<1000): 8-32              │
│   Need more updates per epoch            │
│                                          │
│ Large Dataset (>100K): 64-256            │
│   Efficiency matters more                │
│                                          │
│ With Batch Norm: ≥16, prefer ≥32        │
│   Need good batch statistics             │
│                                          │
│ GPU Memory Limited: Smallest that fits   │
...
□ Batch size in [8, 256] range?
□ Learning rate scaled with batch size?
□ Shuffling enabled for training?
□ Using shuffle=True in DataLoader?
□ Batch Norm with batch size ≥16?
□ Last batch size consistent (use drop_last)?
□ Test with larger batches if possible?
□ Monitor gradient norms (too large/small)?
□ Loss oscillating wildly? → Increase batch size
□ Loss barely moving? → Decrease batch size
```

---

**You now understand mini-batch gradient descent completely! 🎉**
...
```
Raw Daily Temperatures (°C):
Day 1:  18°C
Day 2:  23°C  ← Sudden spike!
Day 3:  19°C
Day 4:  20°C
Day 5:  17°C
Day 6:  21°C
...
Day 3 avg = (18 + 23 + 19) / 3 = 20.0°C
Day 4 avg = (23 + 19 + 20) / 3 = 20.7°C
Day 5 avg = (19 + 20 + 17) / 3 = 18.7°C
Day 6 avg = (20 + 17 + 21) / 3 = 19.3°C
...
...
Day 1:  v₁ = 18°C (starting value)

Day 2:  v₂ = 0.9 × v₁ + 0.1 × 23°C
           = 0.9 × 18 + 0.1 × 23
           = 16.2 + 2.3
           = 18.5°C

Day 3:  v₃ = 0.9 × v₂ + 0.1 × 19°C
           = 0.9 × 18.5 + 0.1 × 19
           = 16.65 + 1.9
           = 18.55°C

Day 4:  v₄ = 0.9 × v₃ + 0.1 × 20°C
           = 0.9 × 18.55 + 0.1 × 20
           = 16.695 + 2.0
           = 18.695°C

Day 5:  v₅ = 0.9 × v₄ + 0.1 × 17°C
           = 0.9 × 18.695 + 0.1 × 17
           = 16.8255 + 1.7
           = 18.5255°C
```
...
```
β = 0.9   → 1/(1-0.9) = 1/0.1 = 10 days
β = 0.95  → 1/(1-0.95) = 1/0.05 = 20 days
β = 0.98  → 1/(1-0.98) = 1/0.02 = 50 days
β = 0.99  → 1/(1-0.99) = 1/0.01 = 100 days
β = 0.999 → 1/(1-0.999) = 1/0.001 = 1000 days
```

**Intuition:**
- Small β (e.g., 0.5): Fast adaptation, follows recent values closely
- Large β (e.g., 0.999): Slow adaptation, very smooth but lags behind
...
We'll compute EWA with different β values.
```

---

### Case 1: β = 0.9 (Fast Adaptation)

**Initialization:**
```
v₀ = 0  (or we can use θ₁ = 18)
Let's use v₀ = 0 to show the issue
```
...
v₁ = β·v₀ + (1-β)·θ₁
   = 0.9 × 0 + 0.1 × 18
   = 0 + 1.8
   = 1.8°C  ← Way too low! (true value is 18°C)
```
...
v₃ = 0.9 × 3.92 + 0.1 × 19
   = 3.528 + 1.9
   = 5.428°C  ← Getting closer but slow...
```
...
| Day | θ (actual) | v (EWA, β=0.9) | Error |
|-----|-----------|---------------|-------|
| 1 | 18°C | 1.8°C | -16.2°C |
| 2 | 23°C | 3.92°C | -19.08°C |
| 3 | 19°C | 5.428°C | -13.572°C |
| 4 | 20°C | 6.8852°C | -13.115°C |
| 5 | 17°C | 7.89668°C | -9.103°C |
| 6 | 21°C | 9.40701°C | -11.593°C |
| 7 | 19°C | 10.36631°C | -8.634°C |
| 8 | 18°C | 11.12968°C | -6.870°C |
| 9 | 22°C | 12.21671°C | -9.783°C |
| 10 | 20°C | 12.99504°C | -7.005°C |
...
### Case 2: β = 0.98 (Slow Adaptation)

**Same calculation:**

| Day | θ (actual) | v (EWA, β=0.98) | Error |
|-----|-----------|----------------|-------|
| 1 | 18°C | 0.36°C | -17.64°C |
| 2 | 23°C | 0.8128°C | -22.187°C |
| 3 | 19°C | 1.1765°C | -17.824°C |
| 4 | 20°C | 1.5530°C | -18.447°C |
| 5 | 17°C | 1.8619°C | -15.138°C |
| 6 | 21°C | 2.2647°C | -18.735°C |
| 7 | 19°C | 2.5994°C | -16.401°C |
| 8 | 18°C | 2.8674°C | -15.133°C |
| 9 | 22°C | 3.2501°C | -18.750°C |
| 10 | 20°C | 3.5851°C | -16.415°C |

**Even worse!** Larger β = slower warmup!
...
Sum of weights = (1-β)(1 + β + β² + ... + β^(t-1))
               = (1-β) × (1-β^t)/(1-β)
               = 1 - β^t

For t=1: Sum = 1 - β¹ = 1 - 0.9 = 0.1  ← Only 10% weight!
For t=2: Sum = 1 - β² = 1 - 0.81 = 0.19 ← Only 19% weight!
For t=5: Sum = 1 - β⁵ = 1 - 0.59 = 0.41 ← Only 41% weight!
For t=10: Sum = 1 - β¹⁰ = 1 - 0.35 = 0.65 ← Getting better
For t=100: Sum = 1 - β¹⁰⁰ ≈ 0.9999 ← Finally close to 1!
...
```
    Temperature (°C)
      ...
      25│
        │    ●                    ●
      20│  ●   ●   ●     ●   ●     ● ← Actual temperatures
        │        ●     ●   ●   ●
      15│
        │
      10│
        │
       5│      ╱──────────────────── EWA without bias correction
        │     ╱
       0│────╱
        └────────────────────────────→ Days
         0   2   4   6   8   10
...
- $\beta^t$ = β raised to power t

**Why this works:**

```
Remember: Sum of weights in v_t = 1 - β^t

To make it sum to 1, divide by (1 - β^t)!

Early on (small t):
- β^t is large (e.g., 0.9¹ = 0.9)
- 1 - β^t is small (e.g., 0.1)
- Division by small number → scales UP significantly

Later (large t):
- β^t is tiny (e.g., 0.9¹⁰⁰ ≈ 0)
- 1 - β^t ≈ 1
- Division by ~1 → no scaling needed
```
...
### Temperature Data with β = 0.9

**Without bias correction (same as before):**

| Day t | θ_t | v_t (biased) | 1-β^t | v̂_t (corrected) | True avg |
|-------|-----|-------------|-------|----------------|----------|
| 1 | 18 | 1.8 | 0.1 | 1.8/0.1 = **18.0** | 18.0 ✓ |
| 2 | 23 | 3.92 | 0.19 | 3.92/0.19 = **20.63** | 20.5 ✓ |
| 3 | 19 | 5.428 | 0.271 | 5.428/0.271 = **20.03** | 20.0 ✓ |
| 4 | 20 | 6.8852 | 0.344 | 6.8852/0.344 = **20.01** | 20.0 ✓ |
| 5 | 17 | 7.897 | 0.410 | 7.897/0.410 = **19.26** | 19.4 ✓ |
| 6 | 21 | 9.407 | 0.469 | 9.407/0.469 = **20.06** | 19.8 ✓ |
| 7 | 19 | 10.366 | 0.522 | 10.366/0.522 = **19.86** | 19.7 ✓ |
| 8 | 18 | 11.130 | 0.570 | 11.130/0.570 = **19.52** | 19.5 ✓ |
| 9 | 22 | 12.217 | 0.613 | 12.217/0.613 = **19.93** | 19.8 ✓ |
| 10 | 20 | 12.995 | 0.651 | 12.995/0.651 = **19.96** | 19.9 ✓ |
...
```
Raw EWA:
v₁ = 0.9 × 0 + 0.1 × 18 = 1.8°C

Bias correction factor:
1 - β^t = 1 - 0.9¹ = 1 - 0.9 = 0.1

Corrected EWA:
v̂₁ = v₁ / (1 - β^t)
   = 1.8 / 0.1
   = 18.0°C  ← Exactly the true value! ✓
```
...
(18 + 23 + 19 + 20 + 17) / 5 = 97 / 5 = 19.4°C

Very close! Error = 0.12°C
```
...
For β = 0.9:
- Day 1: Correct by dividing by 0.1 (×10 multiplier!)
- Day 5: Correct by dividing by 0.41 (×2.4 multiplier)
- Day 10: Correct by dividing by 0.65 (×1.5 multiplier)
- Day 20: Correct by dividing by 0.88 (×1.14 multiplier)
- Day 50: Correct by dividing by ~1.0 (×1.0 - no correction needed)
...
## 4. Comparing Different β Values

### Temperature Data with Multiple β:

**Setup:** Same 10 days of temperature

| Day | Actual | β=0.5 | β=0.9 (corrected) | β=0.98 (corrected) | True Avg |
|-----|--------|-------|------------------|-------------------|----------|
    ...
    Temperature
      ...
      16│       |                       ─ ─ β=0.98 (smooth)
        └───────┼────────────────→ Days
                ↑
           Big spike - see how
           each β reacts differently!
           
β=0.5:  Follows spike closely (reactive)
β=0.9:  Moderate response (balanced)
β=0.98: Barely notices spike (smooth)
```
...
```
Batch 1: ∂L/∂w = 0.023
Batch 2: ∂L/∂w = -0.018  ← Different sign!
Batch 3: ∂L/∂w = 0.031
Batch 4: ∂L/∂w = 0.012
Batch 5: ∂L/∂w = -0.008
Batch 6: ∂L/∂w = 0.025
...
```
v := β·v + (1-β)·∂L/∂w  (compute EWA)
v̂ := v / (1 - β^t)      (bias correction)
w := w - α·v̂            (update using smoothed gradient)
```
...
EWA:
v₁ = 0.9 × 0 + 0.1 × 0.023 = 0.0023

Bias correction:
v̂₁ = 0.0023 / (1 - 0.9¹) = 0.0023 / 0.1 = 0.023

Update:
w := 0.250 - 0.1 × 0.023 = 0.250 - 0.0023 = 0.2477
```
...
Update:
w := 0.2477 - 0.1 × 0.00142 = 0.2477 - 0.000142 = 0.247558
```
...
Update:
w := 0.247558 - 0.1 × 0.01234 = 0.247558 - 0.001234 = 0.246324
```
...
| Batch | Gradient | v (raw) | 1-β^t | v̂ (corrected) | Weight Update |
|-------|----------|---------|-------|--------------|---------------|
...
```
Batch 1: v₁ = 0.0023 (way too small!)
Batch 2: v₂ = 0.00027 (even smaller!)
→ Tiny weight updates initially
→ Slow learning at start
```

**With bias correction:**
```
Batch 1: v̂₁ = 0.023 (correct magnitude!)
Batch 2: v̂₂ = 0.0014 (reasonable)
→ Proper-sized weight updates from start
...
Bias correction factor ≈ 1.14 (minimal)
...
```
┌──────────────────────────────────────────┐
│      Bias Correction: When Needed?       │
└──────────────────────────────────────────┘

CRITICAL:
✓ Beginning of training (first ~20 iterations)
✓ When β is large (0.98, 0.99, 0.999)
✓ When initial value v₀ = 0

OPTIONAL:
• After many iterations (>100)
• When β is small (0.5, 0.7)
• When initialized with good v₀

UNNECESSARY:
✗ After convergence
✗ In practice for Adam (often omitted)
✗ When using large datasets with many updates
```
...
| β | t=1 correction | t=5 correction | t=10 correction |
|---|---------------|----------------|-----------------|
| **0.5** | ×2.0 | ×1.03 | ×1.0 |
| **0.9** | ×10 | ×2.4 | ×1.5 |
| **0.98** | ×50 | ×10.5 | ×5.5 |
...
**Larger β → more critical to correct bias!**

---

**Late iterations (t = 100 to 1000):**

| β | t=100 correction | t=500 correction | t=1000 correction |
|---|-----------------|------------------|-------------------|
| **0.5** | ×1.0 | ×1.0 | ×1.0 |
| **0.9** | ×1.0 | ×1.0 | ×1.0 |
| **0.98** | ×1.1 | ×1.0 | ×1.0 |
...
for day, temp in enumerate(temperatures, 1):
    ...
    print(f"  Day {day}: Temp={temp}°C, EWA={avg:.2f}°C")
...
for day, temp in enumerate(temperatures, 1):
    ...
    print(f"  Day {day}: Temp={temp}°C, EWA={avg:.2f}°C, "
          f"True Avg={true_avg:.2f}°C, Error={error:.2f}°C")
```
...
```
Without Bias Correction:
  Day 1: Temp=18°C, EWA=1.80°C
  Day 2: Temp=23°C, EWA=3.92°C
  Day 3: Temp=19°C, EWA=5.43°C
  Day 4: Temp=20°C, EWA=6.89°C
  Day 5: Temp=17°C, EWA=7.90°C
  Day 6: Temp=21°C, EWA=9.41°C
  Day 7: Temp=19°C, EWA=10.37°C
  Day 8: Temp=18°C, EWA=11.13°C
  Day 9: Temp=22°C, EWA=12.22°C
  Day 10: Temp=20°C, EWA=13.00°C

==================================================

With Bias Correction:
  Day 1: Temp=18°C, EWA=18.00°C, True Avg=18.00°C, Error=0.00°C
  Day 2: Temp=23°C, EWA=20.63°C, True Avg=20.50°C, Error=0.13°C
  Day 3: Temp=19°C, EWA=20.03°C, True Avg=20.00°C, Error=0.03°C
  Day 4: Temp=20°C, EWA=20.01°C, True Avg=20.00°C, Error=0.01°C
  Day 5: Temp=17°C, EWA=19.28°C, True Avg=19.40°C, Error=0.12°C
  Day 6: Temp=21°C, EWA=20.06°C, True Avg=19.83°C, Error=0.23°C
  Day 7: Temp=19°C, EWA=19.86°C, True Avg=19.71°C, Error=0.15°C
  Day 8: Temp=18°C, EWA=19.53°C, True Avg=19.50°C, Error=0.03°C
  Day 9: Temp=22°C, EWA=19.93°C, True Avg=19.78°C, Error=0.15°C
  Day 10: Temp=20°C, EWA=19.96°C, True Avg=19.90°C, Error=0.06°C

Bias correction gives accurate estimates from Day 1! ✓
```
...
Epoch 1:
  ...
  Batch 2: gradient=-0.018, w=0.2495, loss=0.598  ← Oscillating!
  Batch 3: gradient=0.031, w=0.2464,  loss=0.494
  Batch 4: gradient=0.012, w=0.2452,  loss=0.512
  ...
  
Weight trajectory:
0.250 → 0.2477 → 0.2495 → 0.2464 → 0.2452 → ...
        ↘       ↗       ↘       ↘
...
#### With EWA (β=0.9, bias-corrected):

```
Epoch 1:
  Batch 1: grad=0.023,  v̂=0.023,  w=0.2477, loss=0.654
  Batch 2: grad=-0.018, v̂=0.0014, w=0.2476, loss=0.598  ← Smoother!
  Batch 3: grad=0.031,  v̂=0.0123, w=0.2463, loss=0.494
  Batch 4: grad=0.012,  v̂=0.0118, w=0.2452, loss=0.512
  ...
  
Weight trajectory:
0.250 → 0.2477 → 0.2476 → 0.2463 → 0.2452 → ...
        ↘       ↘       ↘       ↘
...
│
├─ β = 0.95-0.98
│   If gradients very noisy (small batches)
│
└─ β = 0.99-0.999
...
✓ Using large β (≥0.98)
✓ Making critical decisions based on EWA
✓ β^t won't be negligible for your training length

Can skip bias correction when:
• After warm-up period (~20-50 iterations)
• Using small β (≤0.9)
• Training for many iterations (>1000)
• Implementation simplicity matters more than early accuracy
```
...
### ❌ Mistake 1: Not Using Bias Correction Early
...
### ❌ Mistake 4: Forgetting to Increment t
...
**So v_t is biased by factor (1-β^t)!**

Dividing by (1-β^t) removes this bias:
...
### Proof by Example (β=0.9, μ=20):

```
Expected value of raw v_t:

t=1:  E[v₁] = 20 × (1 - 0.9¹) = 20 × 0.1 = 2.0
t=2:  E[v₂] = 20 × (1 - 0.9²) = 20 × 0.19 = 3.8
t=3:  E[v₃] = 20 × (1 - 0.9³) = 20 × 0.271 = 5.42
t=5:  E[v₅] = 20 × (1 - 0.9⁵) = 20 × 0.410 = 8.2
t=10: E[v₁₀] = 20 × (1 - 0.9¹⁰) = 20 × 0.651 = 13.02

All biased downward!

Expected value of corrected v̂_t:

E[v̂_t] = E[v_t] / (1 - β^t)
       = μ(1-β^t) / (1-β^t)
       = μ
       = 20  ← Correct for all t! ✓
```
...
PARAMETERS:
- β ∈ (0,1): Decay parameter
  • Large β (0.98): Very smooth, slow adaptation
  • Small β (0.5): Less smooth, fast adaptation
  • Typical: 0.9 (balances smoothing and responsiveness)

WINDOW: Approximates averaging over 1/(1-β) values
...
PROBLEM: v₀ = 0 → early estimates biased low

FORMULA: v̂_t = v_t / (1 - β^t)

EFFECT:
- Removes initialization bias
- Critical for first ~20 iterations
- Becomes negligible as t → ∞

WHEN TO USE:
✓ Beginning of training
✓ Large β values (≥0.98)
✓ When accuracy matters early on

WHEN TO SKIP:
• After warm-up (~50 iterations)
• Small β (≤0.9)
...
**Expected Value (if θ_t ≈ constant μ):**
...
```
✓ Use β=0.9 as default (averages ~10 values)
✓ Always use bias correction for first ~20 steps
✓ For variance estimates, use larger β (0.99 or 0.999)
✓ Initialize v₀ = 0 (simpler than v₀ = θ₁)
✓ Monitor convergence - adjust β if needed

✗ Don't use β ≥ 1.0 (won't update!)
✗ Don't use β < 0.5 (loses smoothing benefit)
✗ Don't forget to increment t for bias correction
✗ Don't skip bias correction with large β values
```
...
1. Momentum Optimization
   EWA of gradients: v_t = β·v_{t-1} + (1-β)·∇L
   β = 0.9 (typical)
   
2. RMSprop
   EWA of squared gradients: s_t = β·s_{t-1} + (1-β)·(∇L)²
   β = 0.999 (typical)
   
3. Adam Optimizer
   First moment (momentum): m_t with β₁ = 0.9
   Second moment (RMSprop): v_t with β₂ = 0.999
   Both use bias correction!
   
4. Learning Rate Scheduling
   Smooth decay: lr_t = β·lr_{t-1} + (1-β)·lr_target
   β = 0.95 (typical)
```

---

**You now understand exponentially weighted averages and bias correction! 🎉**
...
v_t = β·v_{t-1} + ∇L_t
    = ∇L_t + β·∇L_{t-1} + β²·∇L_{t-2} + β³·∇L_{t-3} + ...
...
If gradients consistently point same direction:
  → Velocity builds up (acceleration!)
  
If gradients oscillate/cancel:
  → Velocity remains small (damping!)
```
...
|-------|----------|---------------|------------|-------------|
| 1 | 0.08 | w -= 0.1×0.08 = 0.008 | 0.492 | ↓ |
| 2 | 0.09 | w -= 0.1×0.09 = 0.009 | 0.483 | ↓ |
| 3 | 0.07 | w -= 0.1×0.07 = 0.007 | 0.476 | ↓ |
| 4 | 0.10 | w -= 0.1×0.10 = 0.010 | 0.466 | ↓ |
| 5 | -0.02 | w -= 0.1×(-0.02) = -0.002 | 0.468 | ↑ Wrong way! |
| 6 | 0.08 | w -= 0.1×0.08 = 0.008 | 0.460 | ↓ |
| 7 | 0.09 | w -= 0.1×0.09 = 0.009 | 0.451 | ↓ |
| 8 | 0.08 | w -= 0.1×0.08 = 0.008 | 0.443 | ↓ |
| 9 | 0.11 | w -= 0.1×0.11 = 0.011 | 0.432 | ↓ |
| 10 | 0.07 | w -= 0.1×0.07 = 0.007 | 0.425 | ↓ |

**Final:** w changed from 0.500 → 0.425 (total change: -0.075)
...
```
Gradient: ∇L = 0.08
Velocity: v₁ = 0.9×0 + 0.1×0.08 = 0.008
Update: w := 0.500 - 0.1×0.008 = 0.500 - 0.0008 = 0.4992
```

**Batch 2:**
```
Gradient: ∇L = 0.09
Velocity: v₂ = 0.9×0.008 + 0.1×0.09
             = 0.0072 + 0.009
             = 0.0162
Update: w := 0.4992 - 0.1×0.0162 = 0.4992 - 0.00162 = 0.49758
```

**Batch 3:**
```
Gradient: ∇L = 0.07
Velocity: v₃ = 0.9×0.0162 + 0.1×0.07
             = 0.01458 + 0.007
             = 0.02158
Update: w := 0.49758 - 0.1×0.02158 = 0.49758 - 0.002158 = 0.495422
```

**Batch 4:**
```
Gradient: ∇L = 0.10
Velocity: v₄ = 0.9×0.02158 + 0.1×0.10
             = 0.019422 + 0.01
             = 0.029422
Update: w := 0.495422 - 0.1×0.029422 = 0.492480
```

**Batch 5 (oscillation!):**
```
Gradient: ∇L = -0.02  ← Negative!
Velocity: v₅ = 0.9×0.029422 + 0.1×(-0.02)
             = 0.0264798 - 0.002
             = 0.0244798  ← Still positive! Momentum smoothed it!
Update: w := 0.492480 - 0.1×0.0244798 = 0.489932
```

**Batch 6:**
```
Gradient: ∇L = 0.08
Velocity: v₆ = 0.9×0.0244798 + 0.1×0.08
             = 0.02203182 + 0.008
             = 0.03003182
Update: w := 0.489932 - 0.1×0.03003182 = 0.486929
```

**Continuing through all 10 batches:**

| Batch | Gradient | Velocity | Weight | Loss Direction |
|-------|----------|----------|--------|----------------|
| 1 | 0.08 | 0.008 | 0.4992 | ↓ |
| 2 | 0.09 | 0.0162 | 0.4976 | ↓↓ Accelerating! |
| 3 | 0.07 | 0.0216 | 0.4954 | ↓↓ |
| 4 | 0.10 | 0.0294 | 0.4925 | ↓↓ |
| 5 | -0.02 | 0.0245 | 0.4899 | ↓ Smooth! |
| 6 | 0.08 | 0.0300 | 0.4869 | ↓↓ |
| 7 | 0.09 | 0.0360 | 0.4833 | ↓↓↓ Building speed! |
| 8 | 0.08 | 0.0404 | 0.4793 | ↓↓↓ |
| 9 | 0.11 | 0.0474 | 0.4745 | ↓↓↓ |
| 10 | 0.07 | 0.0496 | 0.4696 | ↓↓↓ Maximum velocity! |

**Final:** w changed from 0.500 → 0.4696 (total change: -0.0304)
...
```
Standard GD:  0.500 → 0.425 (change: -0.075) ✓ More change
Momentum GD:  0.500 → 0.470 (change: -0.030) ✗ Less change?

Wait, that seems wrong! Let me recalculate...

Actually, I made an error. Let me use the common formulation without (1-β):
```

**Momentum (common formulation: v_t = β·v_{t-1} + ∇L):**

| Batch | Gradient | Velocity | Weight Update | New Weight |
|-------|----------|----------|---------------|------------|
| 1 | 0.08 | 0.08 | 0.1×0.08 = 0.008 | 0.492 |
| 2 | 0.09 | 0.9×0.08+0.09 = 0.162 | 0.1×0.162 = 0.0162 | 0.4758 |
| 3 | 0.07 | 0.9×0.162+0.07 = 0.2158 | 0.1×0.2158 = 0.02158 | 0.45422 |
| 4 | 0.10 | 0.9×0.2158+0.10 = 0.29422 | 0.1×0.29422 = 0.029422 | 0.42480 |
| 5 | -0.02 | 0.9×0.29422-0.02 = 0.2448 | 0.1×0.2448 = 0.02448 | 0.40032 |
| 6 | 0.08 | 0.9×0.2448+0.08 = 0.30032 | 0.1×0.30032 = 0.030032 | 0.37029 |
| 7 | 0.09 | 0.9×0.30032+0.09 = 0.36029 | 0.1×0.36029 = 0.036029 | 0.33426 |
| 8 | 0.08 | 0.9×0.36029+0.08 = 0.40426 | 0.1×0.40426 = 0.040426 | 0.29383 |
| 9 | 0.11 | 0.9×0.40426+0.11 = 0.47383 | 0.1×0.47383 = 0.047383 | 0.24645 |
| 10 | 0.07 | 0.9×0.47383+0.07 = 0.49645 | 0.1×0.49645 = 0.049645 | 0.19680 |

**Final:** w changed from 0.500 → 0.197 (total change: -0.303)

**Much faster! ✓** 4× more progress than standard GD!
...
→ Velocity builds up exponentially
→ Takes larger and larger steps
→ Fast convergence!

When gradients oscillate:
→ Velocity dampens oscillations
→ Smooths the path
...
| 2 | -0.0187 | 0.24953 | +0.00187 ← Oscillation |
| 3 | 0.0312 | 0.24641 | -0.00312 |
| 4 | 0.0145 | 0.24496 | -0.00145 |
| 5 | -0.0089 | 0.24585 | +0.00089 ← Oscillation |
...
```
Gradient: ∇L = 0.0234
Velocity: v₁ = 0.9×0 + 0.0234 = 0.0234
Weight: w := 0.250 - 0.1×0.0234 = 0.24766
```

**Batch 2:**
```
Gradient: ∇L = -0.0187  ← Opposite sign!
Velocity: v₂ = 0.9×0.0234 + (-0.0187)
            = 0.02106 - 0.0187
            = 0.00236  ← Momentum dampened oscillation!
Weight: w := 0.24766 - 0.1×0.00236 = 0.24742
```

**Batch 3:**
```
Gradient: ∇L = 0.0312
Velocity: v₃ = 0.9×0.00236 + 0.0312
            = 0.002124 + 0.0312
            = 0.033324
Weight: w := 0.24742 - 0.1×0.033324 = 0.24409
```

**Batch 4:**
```
Gradient: ∇L = 0.0145
Velocity: v₄ = 0.9×0.033324 + 0.0145
            = 0.0299916 + 0.0145
            = 0.0444916
Weight: w := 0.24409 - 0.1×0.0444916 = 0.23964
```

**Batch 5:**
```
Gradient: ∇L = -0.0089  ← Another oscillation!
Velocity: v₅ = 0.9×0.0444916 + (-0.0089)
            = 0.04004244 - 0.0089
            = 0.03114244  ← Dampened again!
Weight: w := 0.23964 - 0.1×0.03114244 = 0.23653
```
...
|-------|----------|----------|--------|---------|
| 1 | 0.0234 | 0.0234 | 0.24766 | ↓ |
| 2 | -0.0187 | 0.0024 | 0.24742 | ↓ No reversal! |
| 3 | 0.0312 | 0.0333 | 0.24409 | ↓↓ |
| 4 | 0.0145 | 0.0445 | 0.23964 | ↓↓ |
| 5 | -0.0089 | 0.0311 | 0.23653 | ↓ Dampened! |
| 6 | 0.0256 | 0.0536 | 0.23117 | ↓↓↓ |
| ... | ... | ... | ... | ... |
| 32 | 0.0198 | 0.0823 | 0.18456 | ↓↓↓ |

**Statistics:**
```
Total change: 0.250 → 0.185 (-0.065)
Average step size: 0.00203 (3.6× larger than standard GD!)
Number of oscillations: 0/32 (no reversals! ✓)
Training loss: 0.693 → 0.543 (better than standard GD!)

Momentum made 3.6× more progress! ✓
```
...
### Formulation 1: Classic (with (1-β) factor)
...
**Typical values:**
- β = 0.9 (averages ~10 gradients)
- α = 0.1-1.0 (similar to standard GD)

---

### Formulation 2: Modern (without (1-β) factor)
...
**Typical values:**
- β = 0.9 (same momentum coefficient)
- α = 0.01-0.001 (much smaller! $(1-\beta)$ absorbed into $\alpha$)
...
```
PyTorch default (Formulation 2):
optimizer = torch.optim.SGD(
    model.parameters(),
    lr=0.001,      # Small LR
    momentum=0.9   # This is β
)

TensorFlow default (Also Formulation 2):
optimizer = tf.keras.optimizers.SGD(
    learning_rate=0.001,
    momentum=0.9
)

Both use Formulation 2!
Just remember: learning rate should be smaller!
```
...
**Training with different β values:**

| β | Velocity Decay | Final Loss | Train Acc | Test Acc | Epochs to 90% | Oscillations |
|---|---------------|-----------|-----------|----------|---------------|--------------|
| **0** | No momentum | 0.342 | 87% | 85% | 25 | Many |
| **0.5** | Moderate | 0.298 | 89% | 88% | 18 | Some |
| **0.7** | More | 0.267 | 91% | 89% | 14 | Few |
| **0.9** | High | 0.234 | 93% | **91%** | **10** | Very few ✓ |
...
β=0.9: Builds to stable level
β=0.5: Doesn't build enough momentum
β=0.99: Builds too much, overshoots
```
...
```
        w₂
         ↑
     100│
        │  ╲│╱  ← Steep sides (large gradients)
      50│   │
        │   │   ← Ravine bottom (shallow, slow)
       0│───●──────────────────────→ w₁
        │      ↓ Want to go this way (toward minimum)
     -50│   │
        │  ╱│╲
    -100│
         0  100 200 300 400
...
```
Batch 1: ∇w₁ = 0.01, ∇w₂ = 5.0  → Move mostly vertical!
Batch 2: ∇w₁ = 0.01, ∇w₂ = -4.8 → Oscillate back!
Batch 3: ∇w₁ = 0.01, ∇w₂ = 4.9  → Oscillate again!
...

After 100 batches:
w₁: 0 → 1.0 (moved 1.0 toward minimum)
w₂: 0 → 2.3 (useless vertical movement)
...
```
Batch 1: ∇w₁ = 0.01, ∇w₂ = 5.0
  v_w₁ = 0.01, v_w₂ = 5.0
  
Batch 2: ∇w₁ = 0.01, ∇w₂ = -4.8
  v_w₁ = 0.9×0.01 + 0.01 = 0.019 (building!)
  v_w₂ = 0.9×5.0 - 4.8 = 0.7 (dampened!)
  
Batch 3: ∇w₁ = 0.01, ∇w₂ = 4.9
  v_w₁ = 0.9×0.019 + 0.01 = 0.0271 (still building!)
  v_w₂ = 0.9×0.7 + 4.9 = 5.53 (oscillating but controlled)
  
...

After 100 batches:
w₁: 0 → 15.2 (moved 15× farther! ✓)
w₂: 0 → 1.1 (vertical oscillations canceled out! ✓)
...
"Where am I now?"          "Where will I be after momentum jump?"
        ...
        Current position              ╱──→ Look-ahead position
              ●                     ●      (where momentum takes us)
               ╲                     ╲
                ↓                     ↓
...
**Standard Momentum:**
```
Current gradient at w=0.10: ∇L = 0.05 (says move left)
Velocity: v = 0.9×(-0.08) + 0.05 = -0.072 + 0.05 = -0.022
Update: w := 0.10 - 0.1×(-0.022) = 0.10 + 0.0022 = 0.1022
...
**Nesterov Momentum:**
```
Look-ahead position: w_lookahead = 0.10 - 0.1×0.9×(-0.08)
                                  = 0.10 + 0.0072
                                  = 0.1072

Gradient at look-ahead: ∇L(0.1072) = -0.03 (says move RIGHT!)
Velocity: v = 0.9×(-0.08) + (-0.03) = -0.072 - 0.03 = -0.102
Update: w := 0.10 - 0.1×(-0.102) = 0.10 + 0.0102 = 0.1102
...
### Choosing β (Momentum Coefficient):

| β Value | Effective Window | Use Case | Characteristics |
|---------|-----------------|----------|-----------------|
...
| **0.9** | ~10 gradients | **Default choice** ✓ | Best for most problems |
...
```
┌─────────────────────────────────────────┐
│      Choosing Momentum Parameters       │
└─────────────────────────────────────────┘

Start with β = 0.9 (default)

Is training oscillating?
├─ YES → Increase β (0.95 or 0.99)
│         More damping
│
└─ NO → Continue

Is convergence slow?
├─ YES → Check if using momentum at all!
│         ├─ NO → Add momentum (β=0.9)
│         └─ YES → Maybe try Nesterov
│
└─ NO → Continue

Overshooting minimum?
├─ YES → Decrease β (0.7 or 0.5)
│         OR decrease learning rate
│
└─ NO → You're good! ✓

Very deep network (>20 layers)?
├─ YES → Use β = 0.95 or 0.99
│         Helps gradients flow through depth
│
└─ NO → β = 0.9 is fine
```
...
Epoch  6: Loss=0.3012, Acc=91.41%  ← Already better!
...
```
Increase β (to 0.95-0.99) if:
├─ Training very deep network (>50 layers)
├─ Loss oscillates heavily
├─ Using very small batches (≤8)
└─ Want maximum smoothing

Decrease β (to 0.5-0.7) if:
├─ Overshooting minimum
├─ Loss bouncing around optimal point
├─ Need fine-tuning near convergence
└─ Extremely noisy gradients make momentum unstable

Use Nesterov if:
├─ Standard momentum works but want slight improvement
├─ Theoretical guarantees matter
└─ Have computational budget for extra forward pass
```
...
PARAMETERS:
- β ∈ [0,1]: Momentum coefficient
  • β = 0.9 (default, ~10 gradients)
  • β = 0.95-0.99 (very deep networks)
  
- α: Learning rate
  • With (1-β) factor: α = 0.1-1.0
  • Without (1-β) factor: α = 0.01-0.001
...
✗ Extra memory (velocity)
✗ Can overshoot if β too large
```
...
|--------|---------|--------------|-------|---------|-------------|
| **Standard GD** | w -= α∇L | High | 1× | 1× | Baseline |
| **Momentum** | w -= αv | Low | 2-3× | 2× | **Default! ✓** |
| **Nesterov** | Look-ahead | Very Low | 2-4× | 2× | Theoretical optimality |

---

**You now understand gradient descent with momentum! 🎉**

Key insights:
- **Momentum = EWA of gradients**
- **Accelerates convergence** by building velocity
- **Dampens oscillations** by averaging out opposing gradients
- **β = 0.9** works great for most problems
...
Example:
- Frequent features: Small gradients → Need large LR
- Rare features: Large, sparse gradients → Need small LR
...
Large gradients → Decrease effective LR
Small gradients → Increase effective LR
```
...
- Increases effective LR → Faster progress!

Right wheel: On bumpy road  
- Large corrections (gradients)
- RMSprop sees "large variance in updates"
- Decreases effective LR → More stable!

Both wheels now move smoothly! ✓
```
...
```
Weight W₁ (frequent feature):
Batch 1: ∇W₁ = 0.001
Batch 2: ∇W₁ = 0.002
Batch 3: ∇W₁ = 0.001
Batch 4: ∇W₁ = 0.002
→ Small, consistent gradients
→ Needs LARGE learning rate to make progress

Weight W₂ (rare feature):  
Batch 1: ∇W₂ = 0.0
Batch 2: ∇W₂ = 0.0
Batch 3: ∇W₂ = 2.5  ← Spike!
Batch 4: ∇W₂ = 0.0
→ Large, sparse gradients
→ Needs SMALL learning rate to avoid divergence
```

**Standard GD with fixed LR = 0.1:**
```
W₁: Moves 0.1×0.001 = 0.0001 per step (too slow!)
W₂: Moves 0.1×2.5 = 0.25 when it fires (too much! Unstable!)
...
```
W₁: Detects small gradients → Increases effective LR → Faster progress!
W₂: Detects large gradients → Decreases effective LR → Stable updates!
...
2. Measures magnitude/variance
   Small gradients: (0.001)² = 0.000001 → High effective LR
   Large gradients: (2.5)² = 6.25 → Low effective LR
...
```
Parameter w₁: Frequent, small gradients
Parameter w₂: Rare, large gradients

Both start at: w₁ = w₂ = 0.500
Learning rate: α = 0.1
β = 0.9 (using 0.9 for easier arithmetic; typically 0.999)
ε = 10^(-8)
s₁ = s₂ = 0 (initial)
```
...
```
∇L/∂w₁ = 0.002 (small)
∇L/∂w₂ = 0.000 (zero - rare feature not active)
```
...
For w₂:
Effective LR = 0.1 / (0 + 10⁻⁸)
             = 0.1 / 0.00000001
             = 10,000,000 (huge!)
             
But ∇L/∂w₂ = 0, so:
w₂ := 0.500 - 10,000,000 × 0
    = 0.500 (no change)
```

Wait, this is too extreme! Let me use more realistic values and β=0.999:

---

### Realistic Example with β = 0.999:
...
s₁ = 0.999×0 + 0.001×(0.002)² = 0.000000004
s₂ = 0.999×0 + 0.001×(0)² = 0

Adaptive LR for w₁: 0.1 / (√0.000000004 + 10⁻⁸) ≈ 0.1 / 0.0000632 ≈ 1582

w₁ := 0.500 - 1582×0.002 = 0.500 - 3.164

This is exploding! The issue is we need bias correction AND
the learning rate needs to be much smaller with RMSprop.

Let me use α = 0.001 (more realistic):
```

---

### Corrected Example (α = 0.001, β = 0.999):

**Batch 1:**
```
∇w₁ = 0.002, ∇w₂ = 0.0

s₁ = 0.001×(0.002)² = 0.000000004
s₂ = 0

With bias correction:
ŝ₁ = s₁/(1-0.999¹) = 0.000000004/0.001 = 0.000004

Adaptive update for w₁:
w₁ := 0.500 - 0.001/√(0.000004 + 10⁻⁸) × 0.002
    = 0.500 - 0.001/0.002 × 0.002
    = 0.500 - 0.5 × 0.002
    = 0.500 - 0.001
    = 0.499
...
s₁ = 0.999×0.000000004 + 0.001×(0.0018)²
   = 0.0000000039996 + 0.00000000324
   = 0.0000000072396

ŝ₁ = 0.0000000072396 / (1-0.999²) = 0.0000036

w₁ := 0.499 - 0.001/√0.0000036 × 0.0018
    = 0.499 - 0.001/0.0019 × 0.0018
    = 0.499 - 0.526 × 0.0018
    = 0.499 - 0.000947
    = 0.498053
```
...
**Batch 3:**
```
∇w₁ = 0.0021, ∇w₂ = 2.5  ← Rare feature fires!

s₁ = 0.999×0.0000000072396 + 0.001×(0.0021)²
...
ŝ₂ = 0.00625 / (1-0.999³) = 0.00625/0.002997 = 2.086

For w₁:
Effective LR = 0.001 / √0.000000011642 ≈ 0.001 / 0.0001079 ≈ 9.27
w₁ := 0.498053 - 9.27 × 0.0021 = 0.498053 - 0.0195 = 0.478553

For w₂:
Effective LR = 0.001 / √2.086 ≈ 0.001 / 1.444 ≈ 0.000692
w₂ := 0.500 - 0.000692 × 2.5 = 0.500 - 0.00173 = 0.49827
```

**Key observation:**
```
w₁ (small gradients): Effective LR ≈ 9.27 (boosted!)
w₂ (large gradients): Effective LR ≈ 0.0007 (reduced!)

RMSprop automatically adapted the learning rates! ✓
```
...
| Batch | ∇w₁ | ∇w₂ | w₁ | w₂ | Notes |
|-------|-----|-----|----|----|-------|
| 1 | 0.002 | 0.0 | 0.499998 | 0.500 | Tiny step for w₁ |
| 2 | 0.0018 | 0.0 | 0.499996 | 0.500 | Still tiny |
| 3 | 0.0021 | 2.5 | 0.499994 | 0.4975 | w₂ jumped! |
| 4 | 0.0019 | 0.0 | 0.499992 | 0.4975 | w₁ barely moves |
| 5 | 0.0020 | 0.0 | 0.499990 | 0.4975 | Frustratingly slow |
| 6 | 0.0022 | 0.0 | 0.499988 | 0.4975 | ... |
| 7 | 0.0018 | 3.1 | 0.499986 | 0.49440 | w₂ jumped again! |
...
**Result:**
```
w₁: Barely moved! (0.500 → 0.49998, change = 0.00002)
    Too slow! Need higher LR!
    
w₂: Wild jumps! (0.500 → 0.494, change = 0.006)  
    Unstable! Need lower LR!

Can't find one LR that works for both! ✗
```

---

### RMSprop (α = 0.001, β = 0.999):

| Batch | ∇w₁ | ∇w₂ | s₁ | s₂ | Eff LR₁ | Eff LR₂ | w₁ | w₂ |
|-------|-----|-----|----|----|---------|---------|----|----|
...
**Result:**
```
w₁: Moved steadily! (0.500 → 0.495, change = 0.005)
    Effective LR adapted from 0.5 → 0.16
    Much faster than standard GD! ✓
    
w₂: Stable updates! (0.500 → 0.498, change = 0.002)
    Effective LR ~0.0003 when gradients fire
    No wild jumps despite large gradients! ✓

RMSprop found good learning rates for both! ✓
```
...
→ Small effective learning rate
→ Stable updates

Small RMS (parameter rarely updates):
→ √s_t is small
→ Divide by small number  
→ Large effective learning rate
→ Faster progress
```
...
```
1000 features → 100 hidden → 2 output
Dataset: 1024 images, batch size 32
Training: 10 epochs
```
...
|--------|-------------|--------------|---------------|-----------|------|-------|
| **Standard GD** | α=0.1 | 0.559 | 0.499 | 79.4% | 2.1s | Baseline |
| **Momentum** | α=0.01, β=0.9 | 0.340 | 0.223 | 93.9% | 2.3s | 2× faster convergence |
| **RMSprop** | α=0.001, β=0.999 | 0.312 | 0.198 | 94.8% | 2.4s | Best final performance |
...
optimizer = optim.RMSprop(
    ...
    alpha=0.99,        # Decay rate (this is β, confusingly named!)
...
**Note:** PyTorch calls β "alpha" in RMSprop! Confusing but standard.
...
|-----------|--------|---------------|-------|--------|
| **Learning Rate** | α | 0.001 | 0.0001-0.01 | Base LR for all parameters |
| **Decay Rate** | β | 0.999 | 0.99-0.9999 | How much history to keep |
| **Epsilon** | ε | 10⁻⁸ | 10⁻¹⁰ to 10⁻⁶ | Numerical stability |
...
```
┌─────────────────────────────────────────┐
│      RMSprop Hyperparameter Guide       │
└─────────────────────────────────────────┘

Learning Rate (α):
├─ Start with 0.001 (default)
├─ Too slow? → Increase to 0.01
├─ Unstable? → Decrease to 0.0001
└─ Range: [0.0001, 0.01]

Decay Rate (β):
├─ Default: 0.999
├─ Very noisy gradients? → 0.9999 (more smoothing)
├─ Need fast adaptation? → 0.99 (less history)
└─ Almost always: 0.999 works well

Epsilon (ε):
├─ Default: 1e-8
├─ Numerical issues? → 1e-6 (larger)
...
✓ Mini-batch training with small batches
...
### ❌ Mistake 2: Wrong β value

```python
# BAD: Using momentum's β for RMSprop
optimizer = optim.RMSprop(
    model.parameters(),
    lr=0.001,
...
### ❌ Mistake 3: Forgetting ε can matter

```python
# PROBLEMATIC: ε too small
...
- β = 0.999 (decay for squared gradients)
...
DISADVANTAGES:
✗ More memory (stores s_t for each parameter)
✗ More hyperparameters
✗ Can struggle with sparse gradients
```
...
|--------|--------------|---------------------|-------|---------|----------|
| **Standard GD** | None | None | 1× | 1× | Baseline |
| **Momentum** | None | Yes | 2-3× | 2× | General purpose |
| **RMSprop** | Yes | Partial | 2-4× | 2× | RNNs, varying scales |
| **Adam** | Yes | Yes | 3-5× | 3× | **Default choice** ✓ |
...
**You now understand momentum and RMSprop! 🎉**

Key insights:
- **Momentum** accelerates by accumulating gradients (EWA of ∇L)
- **RMSprop** adapts learning rate by tracking gradient magnitude (EWA of (∇L)²)
- Both use exponentially weighted averages (with different β values!)
