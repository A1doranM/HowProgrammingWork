...
if (cluster.isMaster) {
  ...
  // Ид состоит из четырех чисел, мы разбиваем его на 4 числа, затем сдвигаем на 8 бит, и сохраняем
  // Это делается так как если склеить 4 числа ид то они вылезут за размер инта.
  ...
  // Таким образом пул из некторого числа номеров IP будут попадать на определенный воркер воркеру.
  ...
  process.on("message", (message, socket) => {
    if (message.name === "socket") {
      socket.server = server; // Сокету добавляем свойство сервер, так как старый сервер к которому он был привязан находится в другом аддресном пространстве
